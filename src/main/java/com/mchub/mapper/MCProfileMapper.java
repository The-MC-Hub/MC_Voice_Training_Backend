...
public interface MCProfileMapper {
  ...
  @Mapping(
      target = "ratesMin",
      expression = "java(profile.getRates() != null ? profile.getRates().getMin() : 0)")
  @Mapping(
      target = "ratesMax",
      expression = "java(profile.getRates() != null ? profile.getRates().getMax() : 0)")
  ...
  @Mapping(
      target = "lastActive",
      expression =
          "java(profile.getLastActive() != null ? profile.getLastActive().toString() : null)")
  ...
  @Mapping(
      target = "socialLinks",
      expression =
          "java(profile.getSocialLinks() != null ? new com.mchub.dto.MCProfileResponseDTO.SocialLinksDTO(profile.getSocialLinks().getYoutube(), profile.getSocialLinks().getTiktok(), profile.getSocialLinks().getFacebook(), profile.getSocialLinks().getZalo()) : null)")
  ...
  default java.util.List<MCProfileResponseDTO.EventEntryDTO> toEventEntryDTOs(
      java.util.List<MCProfile.EventEntry> events) {
    if (events == null) return null;
    return events.stream()
        .map(
            e ->
                new MCProfileResponseDTO.EventEntryDTO(
                    e.getId(),
                    e.getTitle(),
                    e.getDescription(),
                    e.getEventType(),
