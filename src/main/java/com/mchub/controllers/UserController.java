...
public class UserController {
  ...
  /**
   * Freeze is consumed automatically in processLoginStreak() when a user logs in after skipping
   * exactly one day (gap==2) — there is no separate action to take here. Both routes below just
   * report the current freeze status so the client can display it; POST is kept for backward
   * compatibility with existing callers, GET is the more semantically correct alias for a read-only
   * status check.
   */
