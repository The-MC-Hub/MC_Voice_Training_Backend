...
public class BookingController {
    ...
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(
            ApiResponse.success(
                ...
                enrichWithUserInfo(bookingMapper.toResponseDTO(saved))));
  }
  ...
  public ResponseEntity<ApiResponse<List<BookingResponseDTO>>> getMyBookings(
    ...
    List<Booking> list =
        "mc".equalsIgnoreCase(role)
            ? bookingService.getMCBookings(userId)
            : bookingService.getClientBookings(userId);
    List<BookingResponseDTO> dtos =
        list.stream().map(bookingMapper::toResponseDTO).map(this::enrichWithUserInfo).toList();
  ...
  public ResponseEntity<ApiResponse<BookingResponseDTO>> getBookingById(@PathVariable String id) {
    ...
    return ResponseEntity.ok(
        ApiResponse.success(enrichWithUserInfo(bookingMapper.toResponseDTO(b))));
  }

  @PutMapping("/{id}/status")
  public ResponseEntity<ApiResponse<BookingResponseDTO>> updateStatus(
      @PathVariable String id, @RequestBody @Valid UpdateBookingStatusRequest req) {
    String mcId = SecurityUtils.getCurrentUserId();
    Booking updated =
        bookingService.updateStatus(
            id, req.getStatus(), mcId, req.getPrice(), req.getRejectionReason());
    return ResponseEntity.ok(
        ApiResponse.success(
            "Update successful", enrichWithUserInfo(bookingMapper.toResponseDTO(updated))));
  }
  ...
  public ResponseEntity<ApiResponse<BookingResponseDTO>> cancelBooking(@PathVariable String id) {
    ...
    return ResponseEntity.ok(
        ApiResponse.success(
            ...
            enrichWithUserInfo(bookingMapper.toResponseDTO(cancelled))));
  }
