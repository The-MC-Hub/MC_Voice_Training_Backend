...
public class AdminSocialPostController {
  ...
  public ResponseEntity<ApiResponse<List<SocialPostDTO>>> listAll() {
    return ResponseEntity.ok(
        ApiResponse.success("Social posts retrieved", socialPostService.getAllPostsAdmin()));
  }
  ...
  public ResponseEntity<ApiResponse<SocialPostDTO>> create(
    ...
    return ResponseEntity.ok(
        ApiResponse.success("Social post created", socialPostService.createPost(request)));
  }

  @PutMapping("/{id}")
  public ResponseEntity<ApiResponse<SocialPostDTO>> update(
      @PathVariable String id, @Valid @RequestBody SaveSocialPostRequest request) {
    return ResponseEntity.ok(
        ApiResponse.success("Social post updated", socialPostService.updatePost(id, request)));
  }
  ...
  public ResponseEntity<ApiResponse<SocialPostDTO>> toggle(@PathVariable String id) {
    return ResponseEntity.ok(
        ApiResponse.success("Social post toggled", socialPostService.toggleActive(id)));
  }
}
