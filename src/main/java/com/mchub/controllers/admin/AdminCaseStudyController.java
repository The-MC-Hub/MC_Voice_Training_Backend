...
public class AdminCaseStudyController {
  ...
  public ResponseEntity<ApiResponse<List<CaseStudy>>> listAll() {
    return ResponseEntity.ok(
        ApiResponse.success("Case studies retrieved", caseStudyRepository.findAll()));
  }
  ...
  public ResponseEntity<ApiResponse<CaseStudy>> create(@RequestBody CaseStudy request) {
    request.setId(null);
    return ResponseEntity.ok(
        ApiResponse.success("Case study created", caseStudyRepository.save(request)));
  }

  @PutMapping("/{id}")
  public ResponseEntity<ApiResponse<CaseStudy>> update(
      @PathVariable String id, @RequestBody CaseStudy request) {
    caseStudyRepository
        .findById(id)
        .orElseThrow(
            () -> new AppException(ErrorCode.COURSE_NOT_FOUND, "Case study not found: " + id));
    request.setId(id);
    return ResponseEntity.ok(
        ApiResponse.success("Case study updated", caseStudyRepository.save(request)));
  }
