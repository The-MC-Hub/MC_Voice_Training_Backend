...
public class AdminPlanController {
  ...
  public ResponseEntity<ApiResponse<PlanDefinition>> updatePlan(
      @PathVariable String id, @RequestBody PlanDefinition updated) {
    return ResponseEntity.ok(
        ApiResponse.success("Plan updated", planService.updatePlan(id, updated)));
  }
  ...
  public ResponseEntity<ApiResponse<List<DiscountCode>>> getAllDiscounts() {
    return ResponseEntity.ok(
        ApiResponse.success("Discounts retrieved", planService.getAllDiscounts()));
  }

  @PostMapping("/discounts")
  public ResponseEntity<ApiResponse<DiscountCode>> createDiscount(
      @RequestBody DiscountCode discount) {
    discount.setId(null); // ensure new document
    discount.setUsedCount(0);
    return ResponseEntity.ok(
        ApiResponse.success("Discount created", planService.saveDiscount(discount)));
  }

  @PutMapping("/discounts/{id}")
  public ResponseEntity<ApiResponse<DiscountCode>> updateDiscount(
      @PathVariable String id, @RequestBody DiscountCode updated) {
    return ResponseEntity.ok(
        ApiResponse.success("Discount updated", planService.updateDiscount(id, updated)));
  }
