...
public class VoiceController {
  ...
  public ResponseEntity<ApiResponse<VoiceLessonResponseDTO>> setSampleAudio(
      @PathVariable String id, @RequestParam(required = false) MultipartFile audio) {
    VoiceLessonResponseDTO dto = voiceService.setSampleAudio(id, audio);
    return ResponseEntity.ok(
        ApiResponse.success(
            audio == null || audio.isEmpty() ? "Sample audio cleared" : "Sample audio uploaded",
            dto));
  }
  ...
  private List<VoiceLesson.EvaluationCriteria> parseCriteria(String json) {
    ...
    try {
      return objectMapper.readValue(
          json, new TypeReference<List<VoiceLesson.EvaluationCriteria>>() {});
    } catch (Exception e) {
  ...
  private void validateAudioFile(MultipartFile audioFile) {
    ...
    if (ctBase.isEmpty() || !ALLOWED_AUDIO_TYPES.contains(ctBase)) {
      throw new AppException(
          ErrorCode.VALIDATION_FAILED, "Chỉ hỗ trợ file audio (wav, mp3, webm, ogg, m4a)");
    }
  ...
  public ResponseEntity<ApiResponse<List<VoiceLessonResponseDTO>>> getAllLessons(
    ...
    List<VoiceLessonResponseDTO> lessons =
  ...
  public ResponseEntity<ApiResponse<VoiceLessonResponseDTO>> getLessonById(
      @PathVariable String id) {
  ...
  public ResponseEntity<ApiResponse<PracticeSessionResponseDTO>> analyzePractice(
      @RequestParam String lessonId, @RequestParam MultipartFile audioFile) {
  ...
  public ResponseEntity<ApiResponse<List<PracticeSessionResponseDTO>>> getHistory(
  ...
  /**
   * GET /api/v1/voice/practice/history/{userId}/lesson/{lessonId} — sessions for one lesson, oldest
   * first
   */
  ...
  public ResponseEntity<ApiResponse<List<PracticeSessionResponseDTO>>> getLessonHistory(
    ...
    return ResponseEntity.ok(
        ApiResponse.success(voiceService.getUserLessonHistory(userId, lessonId)));
  }

  @PostMapping("/proxy/analyze-voice")
  @PreAuthorize("hasAuthority('MC')")
  public ResponseEntity<?> proxyAnalyzeVoice(
      @RequestParam MultipartFile audioFile, @RequestParam(required = false) String scriptOrigin) {
  ...
  public ResponseEntity<?> analyzeGuestVoice(
    ...
    if (setting != null) {
      try {
        cooldownHours = Integer.parseInt(setting.getValue());
      } catch (Exception ignored) {
      }
    }

    if (usage != null
        && usage.getLastUsedAt().plusHours(cooldownHours).isAfter(java.time.LocalDateTime.now())) {
      throw new AppException(
          ErrorCode.VALIDATION_FAILED,
          "Bạn đã sử dụng lượt thử miễn phí. Vui lòng quay lại sau "
              + cooldownHours
              + " tiếng hoặc đăng ký tài khoản để tiếp tục.");
    }
  ...
  @PreAuthorize("hasAuthority('MC') or hasAuthority('ADMIN')")
  public ResponseEntity<ApiResponse<PracticeSessionResponseDTO>> getPracticeById(
      @PathVariable String id) {
  ...
  public ResponseEntity<ApiResponse<LessonAdaptiveStats>> getAdaptiveStats(
      @PathVariable String id) {
    LessonAdaptiveStats stats = voiceService.getAdaptiveStats(id);
    if (stats == null) {
      return ResponseEntity.ok(
          ApiResponse.success("No adaptive data yet (need 10+ sessions)", null));
    }
