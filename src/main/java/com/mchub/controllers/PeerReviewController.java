...
public class PeerReviewController {
  ...
  /**
   * POST /api/v1/peer-review/request/{practiceSessionId} — MC opts in to have a fellow MC review
   * their practice
   */
  @PostMapping("/request/{practiceSessionId}")
  @PreAuthorize("hasAuthority('MC')")
  public ResponseEntity<ApiResponse<PracticeReviewDTO>> requestReview(
            ...
            .orElseThrow(
                () ->
                    new AppException(
                        ...
                        "Practice session not found: " + practiceSessionId));
    ...
    if (practiceReviewRepository.existsByPracticeSessionId(practiceSessionId)) {
      throw new AppException(
          ErrorCode.VALIDATION_FAILED, "Review already requested for this session");
    }
    PracticeReview review =
        PracticeReview.builder()
            .practiceSessionId(practiceSessionId)
            .revieweeId(userId)
            .status("PENDING")
            .build();
    return ResponseEntity.ok(
        ApiResponse.success("Review requested", toDTO(practiceReviewRepository.save(review))));
  }
  ...
  public ResponseEntity<ApiResponse<PracticeReviewDTO>> submit(
    ...
    PracticeReview review =
        practiceReviewRepository
            .findById(id)
            .orElseThrow(
                () -> new AppException(ErrorCode.RESOURCE_NOT_FOUND, "Review not found: " + id));
    ...
    return ResponseEntity.ok(
        ApiResponse.success("Review submitted", toDTO(practiceReviewRepository.save(review))));
  }
  ...
  @PreAuthorize("hasAuthority('MC')")
  public ResponseEntity<ApiResponse<PracticeReviewDTO>> forSession(
      @PathVariable String practiceSessionId) {
    PracticeReview review =
        practiceReviewRepository.findByPracticeSessionId(practiceSessionId).orElse(null);
    return ResponseEntity.ok(
        ApiResponse.success("Review retrieved", review != null ? toDTO(review) : null));
  }

  private PracticeReviewDTO toDTO(PracticeReview r) {
    PracticeSession session =
        practiceSessionRepository.findById(r.getPracticeSessionId()).orElse(null);
    VoiceLesson lesson =
        session != null ? lessonRepository.findById(session.getLessonId()).orElse(null) : null;
    User reviewee = userRepository.findById(r.getRevieweeId()).orElse(null);
    User reviewer =
        r.getReviewerId() != null ? userRepository.findById(r.getReviewerId()).orElse(null) : null;
