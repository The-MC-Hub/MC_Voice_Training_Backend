...
public class PaymentController {
  ...
  public ResponseEntity<ApiResponse<java.util.List<com.mchub.models.DiscountCode>>>
      getFlashDeals() {
    return ResponseEntity.ok(
        ApiResponse.success("Flash deals retrieved", planService.getActiveFlashDeals()));
  }
  ...
  public ResponseEntity<ApiResponse<java.util.Map<String, Object>>> applyDiscount(
      @RequestParam String code, @RequestParam SubscriptionPlan plan) {
    return ResponseEntity.ok(
        ApiResponse.success("Discount applied", planService.applyDiscount(code, plan)));
  }
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> createPremiumOrder(
    ...
    try {
      ...
      effectiveAmount =
          planDef.getDiscountedPriceVnd() > 0
    ...
    if (discountCode != null && !discountCode.trim().isEmpty()) {
      try {
        ...
        throw new AppException(
            ErrorCode.VALIDATION_FAILED, "Mã giảm giá không hợp lệ: " + e.getMessage());
      }
    }

    // Generate unique orderCode (PayOS requires positive long ≤ 9007199254740991)
    long orderCode =
        System.currentTimeMillis() % 1_000_000_000L * 100
    ...
    if (effectiveAmount <= 0) {
      ...
      return ResponseEntity.ok(
          ApiResponse.success("Plan activated automatically with 100% discount", data));
    }
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> createCourseOrder(
      @RequestParam String courseId, @RequestParam(required = false) String discountCode) {
    ...
    com.mchub.models.Course course =
        courseRepository
            .findById(courseId)
            .orElseThrow(
                () ->
                    new AppException(ErrorCode.COURSE_NOT_FOUND, "Course not found: " + courseId));
    ...
    if (discountCode != null && !discountCode.trim().isEmpty()) {
      throw new AppException(
          ErrorCode.VALIDATION_FAILED, "Mã giảm giá không áp dụng cho mua khóa học lẻ");
    }
    ...
    if (effectiveAmount <= 0) {
      ...
      return ResponseEntity.ok(
          ApiResponse.success("Course granted automatically with 100% discount", data));
    }

    Map<String, Object> checkout;
    try {
      checkout =
          payOSService.createCoursePaymentLink(
              userId, course.getTitle(), orderCode, effectiveAmount);
    } catch (Exception e) {
  ...
  /**
   * Applies a completed subscription-plan transaction to the user, guarding against a
   * late/out-of-order transaction downgrading a plan the user already upgraded to via a more recent
   * transaction. A transaction is only applied if the target plan is the same tier or higher than
   * the user's current plan, OR the user's current plan has already expired (in which case any
   * completed transaction should apply).
   */
  private void applyPlanUpgrade(User user, SubscriptionPlan newPlan) {
    boolean currentPlanActive =
        user.getPlanExpiresAt() != null && user.getPlanExpiresAt().isAfter(LocalDateTime.now());
    boolean isDowngrade =
        newPlan != SubscriptionPlan.DAILY
            && currentPlanActive
            && user.getPlan() != null
    ...
    if (isDowngrade) {
      log.warn(
          ">>> PLAN UPGRADE SKIPPED (would downgrade): user={} currentPlan={} (expires {}) — incoming plan={} ignored",
          user.getEmail(),
          user.getPlan(),
    ...
    notificationService.notify(
        ...
        "Gói "
            + newPlan.name()
            + " của bạn đã được kích hoạt, có hiệu lực đến "
            + user.getPlanExpiresAt()
            + ".",
        "/m/payment",
        true);
    ...
    userRepository
        ...
        .ifPresent(
            user -> {
              ...
              if (!courseEnrollmentRepository.existsByUserIdAndCourseId(
                  tx.getUserId(), tx.getCourseId())) {
                courseEnrollmentRepository.save(
                    com.mchub.models.CourseEnrollment.builder()
                        .userId(tx.getUserId())
                        .courseId(tx.getCourseId())
                        .build());
              }
              log.info(
                  ">>> COURSE PURCHASED: user={} course={}", user.getEmail(), tx.getCourseId());
                ...
                tx -> {
                  if (tx.getStatus() == TransactionStatus.PENDING) {
                    ...
                    if (tx.getCourseId() != null) {
                      ...
                      userRepository
                          .findById(tx.getUserId())
                          .ifPresent(user -> applyPlanUpgrade(user, tx.getPlan()));
                    }
                  }
                });
      }
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> getPaymentStatus(
    ...
    List<PaymentTransaction> txList =
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> simulatePaymentSuccess(
      @RequestParam String userId, @RequestParam(defaultValue = "FULL") SubscriptionPlan plan) {
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> adminCompleteTransaction(
    ...
    User user =
        userRepository
            ...
            .orElseThrow(
                () ->
                    new AppException(
                        ErrorCode.USER_NOT_FOUND, "User not found: " + tx.getUserId()));
    ...
    if (tx.getCourseId() != null) {
      ...
      log.info(
          ">>> ADMIN MANUAL COMPLETE (COURSE): txId={} user={} course={}",
          transactionId,
          user.getEmail(),
          tx.getCourseId());
      return ResponseEntity.ok(
          ApiResponse.success("Transaction completed manually. Course unlocked.", data));
    }

    applyPlanUpgrade(user, tx.getPlan());

    log.info(
        ">>> ADMIN MANUAL COMPLETE: txId={} user={} plan={}",
        transactionId,
        user.getEmail(),
        tx.getPlan());
    ...
    return ResponseEntity.ok(
        ApiResponse.success("Transaction completed manually. Plan activated.", data));
  }
}
