    ...
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(
            ApiResponse.success(
                ...
                enrichWithUserInfo(reviewMapper.toResponseDTO(saved))));
  }

  @GetMapping("/mc/{mcId}")
  public ResponseEntity<ApiResponse<List<ReviewResponseDTO>>> getMCReviews(
      @PathVariable String mcId) {
    List<ReviewResponseDTO> dtos =
        reviewService.getReviewsByMc(mcId).stream()
