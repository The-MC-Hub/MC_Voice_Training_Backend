...
public class CertificateController {
  ...
  public ResponseEntity<ApiResponse<CertificateResponseDTO>> addCertificate(
    ...
    if (mcProfileId == null || mcProfileId.isBlank()) {
      throw new AppException(
          ErrorCode.MC_PROFILE_NOT_FOUND,
          "You need to have an MC profile before adding a certificate");
    }
    Certificate cert = certificateService.addCertificate(mcProfileId, Objects.requireNonNull(req));
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(
            ApiResponse.success(
                "Certificate added successfully", certificateMapper.toResponseDTO(cert)));
  }
  ...
  public ResponseEntity<ApiResponse<List<CertificateResponseDTO>>> getCertificates(
    ...
    List<CertificateResponseDTO> dtos =
        certificateService.getCertificatesByMCProfile(Objects.requireNonNull(mcProfileId)).stream()
  ...
  public ResponseEntity<ApiResponse<CertificateResponseDTO>> verify(@PathVariable String id) {
    ...
    Certificate verified =
        certificateService.verifyCertificate(Objects.requireNonNull(id), adminId);
    return ResponseEntity.ok(
        ApiResponse.success("Verified successfully", certificateMapper.toResponseDTO(verified)));
  }
