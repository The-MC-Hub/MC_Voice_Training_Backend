...
public class AuthController {
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> verifyEmailByToken(
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> verifyOtp(
    ...
    AuthService.LoginResponse resp =
        authService.verifyOtpAndLogin(Objects.requireNonNull(email), Objects.requireNonNull(code));
    return ResponseEntity.ok(
        ApiResponse.success(
            "Email verified successfully",
            Map.of("token", resp.token(), "user", userMapper.toResponseDTO(resp.user()))));
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> login(
    ...
    try {
      AuthService.LoginResponse resp =
          authService.login(
              Objects.requireNonNull(req.getEmail()), Objects.requireNonNull(req.getPassword()));
      auditLogService.log(
          resp.user().getId(), AuditAction.AUTH_LOGIN, "User", resp.user().getId(), null, request);
      try {
        gamificationService.processLoginStreak(resp.user().getId());
      } catch (Exception ignored) {
      }
      return ResponseEntity.ok(
          ApiResponse.success(
      ...
      if (ex.getErrorCode() == ErrorCode.ADMIN_OTP_REQUIRED) {
        ...
        return ResponseEntity.status(202)
            .body(
                ApiResponse.success(
                    "ADMIN_OTP_REQUIRED", Map.of("requiresAdminOtp", true, "email", adminEmail)));
      }
      auditLogService.logError(
          null,
          AuditAction.AUTH_LOGIN_FAILED,
          "User",
          "email=" + req.getEmail() + " " + SecurityUtils.safeMessage(ex),
          request);
      throw ex;
    } catch (Exception ex) {
      auditLogService.logError(
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> loginWithGoogle(
      @RequestBody @Valid GoogleLoginRequest req, HttpServletRequest request) {
    try {
      ...
      return ResponseEntity.status(202)
          .body(
              ApiResponse.success(
                  "Choose a role to finish signing up",
                  Map.of(
      ...
      auditLogService.logError(
          null, AuditAction.AUTH_LOGIN_FAILED, "User", SecurityUtils.safeMessage(ex), request);
      throw ex;
    }
  }
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> completeGoogleRegistration(
      @RequestBody @Valid CompleteGoogleRegistrationRequest req, HttpServletRequest request) {
    AuthService.LoginResponse resp =
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> verifyAdminLoginOtp(
    ...
    String code = body.get("code");
    AuthService.LoginResponse resp =
        authService.verifyAdminLoginOtp(
            Objects.requireNonNull(email), Objects.requireNonNull(code));
    auditLogService.log(
        resp.user().getId(),
        AuditAction.ADMIN_LOGIN_OTP_VERIFY,
        "User",
        resp.user().getId(),
        "{\"method\":\"admin-otp\"}",
        request);
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> linkGoogleAccount(
    ...
    auditLogService.log(
        userId, AuditAction.AUTH_LOGIN, "User", userId, "{\"method\":\"google-link\"}", request);
    return ResponseEntity.ok(
        ApiResponse.success(
            "Google account linked", Map.of("user", userMapper.toResponseDTO(user))));
  }

  @PostMapping("/google/unlink")
  @PreAuthorize("isAuthenticated()")
  public ResponseEntity<ApiResponse<Map<String, Object>>> unlinkGoogleAccount(
      HttpServletRequest request) {
    String userId = SecurityUtils.getCurrentUserId();
    User user = authService.unlinkGoogleAccount(userId);
    auditLogService.log(
        userId, AuditAction.AUTH_LOGIN, "User", userId, "{\"method\":\"google-unlink\"}", request);
    return ResponseEntity.ok(
        ApiResponse.success(
            "Google account unlinked", Map.of("user", userMapper.toResponseDTO(user))));
  }
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> getMe() {
    ...
    return ResponseEntity.ok(
        ApiResponse.success("User retrieved", Map.of("user", userMapper.toResponseDTO(user))));
  }
  ...
  public ResponseEntity<ApiResponse<Map<String, String>>> generateReferralCodeEndpoint() {
    ...
    return ResponseEntity.ok(
        ApiResponse.success("Referral code retrieved", Map.of("referralCode", code)));
  }
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> updateSettings(
    ...
    return ResponseEntity.ok(
        ApiResponse.success(
            "Settings updated successfully", Map.of("user", userMapper.toResponseDTO(user))));
  }
}
