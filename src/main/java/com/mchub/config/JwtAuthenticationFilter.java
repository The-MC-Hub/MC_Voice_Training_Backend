    ...
    try {
      ...
      if (userId != null && SecurityContextHolder.getContext().getAuthentication() == null) {
        ...
        authToken.setDetails(
            new WebAuthenticationDetailsSource().buildDetails(Objects.requireNonNull(request)));
        SecurityContextHolder.getContext().setAuthentication(authToken);

        log.debug(
            "JWT Auth OK — userId={}, role={}, path={}", userId, role, request.getRequestURI());
      }
    } catch (Exception e) {
      log.warn(
          "⚠️ [JWT] Token validation failed for {}: {}",
