...
public final class EntityUtils {
  ...
  public static <T, ID> T getOrThrow(
      CrudRepository<T, ID> repository, ID id, ErrorCode errorCode, String message) {
    return repository.findById(id).orElseThrow(() -> new AppException(errorCode, message));
  }
  ...
  /**
   * Fetch entity by ID or throw USER_NOT_FOUND. Shorthand for the most common lookup:
  ...
  /** Fetch entity by ID or throw RESOURCE_NOT_FOUND. Shorthand for generic resource lookups. */
  public static <T, ID> T getResourceOrThrow(
      CrudRepository<T, ID> repository, ID id, String resourceName) {
    return repository
        .findById(id)
        .orElseThrow(
            () ->
                new AppException(ErrorCode.RESOURCE_NOT_FOUND, resourceName + " not found: " + id));
  }
