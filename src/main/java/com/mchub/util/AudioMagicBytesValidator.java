...
public final class AudioMagicBytesValidator {
  ...
  /**
   * Returns true if the file's first bytes match a known audio format signature. Prevents MIME-type
   * spoofing (e.g., renaming a .html file to .webm).
   */
  public static boolean isValidAudio(InputStream stream) throws IOException {
    ...
    if (header[0] == 0x1A
        && header[1] == 0x45
        && (byte) header[2] == (byte) 0xDF
        && (byte) header[3] == (byte) 0xA3) return true;
