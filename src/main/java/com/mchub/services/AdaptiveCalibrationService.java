...
/**
 * Adaptive calibration — recalibrates passing score and WPM targets for a lesson based on
 * aggregated real user performance data.
 *
 * <p>Triggered asynchronously after each practice session. Only recalibrates once the lesson has
 * reached MIN_SESSIONS data points.
 */
@Service
...
public class AdaptiveCalibrationService {
  ...
  /**
   * Called fire-and-forget after analyzePractice() completes. Skips silently if not enough data
   * yet.
   */
  @Async
  public void calibrateLesson(String lessonId) {
    try {
      ...
      if (count < MIN_SESSIONS) {
        log.debug(
            "[Adaptive] Lesson {} has {} sessions — below threshold {}, skipping",
            lessonId,
            count,
            MIN_SESSIONS);
        return;
      ...
      int calibratedPassing =
          clamp(
              rawCalibrated,
              adminPassing - MAX_PASSING_SCORE_DRIFT,
              adminPassing + MAX_PASSING_SCORE_DRIFT);
      ...
      int calWpmMin =
          clamp(
              avgWpmInt - 15,
              lesson.getTargetWpmMin() - MAX_WPM_DRIFT,
              lesson.getTargetWpmMin() + MAX_WPM_DRIFT);
      int calWpmMax =
          clamp(
              avgWpmInt + 15,
              lesson.getTargetWpmMax() - MAX_WPM_DRIFT,
              lesson.getTargetWpmMax() + MAX_WPM_DRIFT);
      ...
      LessonAdaptiveStats updated =
          LessonAdaptiveStats.builder()
      ...
      log.info(
          "[Adaptive] Lesson {} calibrated — sessions={}, avgScore={:.1f}, passing={}->{}, difficulty={}",
          lessonId,
          sessionCount,
          avgOverall,
          adminPassing,
  ...
  /**
   * Avg/min/max/percentile scores for a lesson, computed via MongoTemplate directly rather than
   * a @Aggregation interface-projection repository method — Spring Data MongoDB's interface
   * projection support for grouped/aggregated queries has a known runtime gap (fails with
   * NullPointerException on Class.isEnum() despite compiling cleanly; the same issue was previously
   * hit and fixed on MinigameResultRepository.getLeaderboard()).
   */
  private Document aggregateScoresByLessonId(String lessonId) {
    ...
    AggregationResults<Document> results =
        mongoTemplate.aggregate(aggregation, "practice_sessions", Document.class);
    ...
    List<Document> pipeline =
        List.of(
            ...
            new Document(
                "$group",
                new Document("_id", "$lessonId")
                    .append(
                        "p25Overall",
                        new Document(
                            "$percentile",
                            new Document("input", "$overallScore")
                                .append("p", List.of(0.25))
                    ...
                    .append(
                        ...
                        new Document(
                            "$percentile",
                            new Document("input", "$overallScore")
                                .append("p", List.of(0.75))
                                .append("method", "approximate")))));
    Document percentiles =
        mongoTemplate
            .getCollection("practice_sessions")
