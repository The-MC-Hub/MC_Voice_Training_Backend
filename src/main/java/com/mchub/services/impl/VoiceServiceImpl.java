...
public class VoiceServiceImpl implements VoiceService {
  ...
  @Override
  public VoiceLessonResponseDTO createLesson(
      String title,
      String content,
      VoiceLessonCategory category,
      String difficulty,
      String description,
      MultipartFile thumbnail,
      String videoUrl,
      List<VoiceLesson.EvaluationCriteria> evaluationCriteria,
    ...
    VoiceLesson lesson =
        VoiceLesson.builder()
            ...
            .videoUrl(videoUrl)
            .evaluationCriteria(
                evaluationCriteria != null ? evaluationCriteria : new java.util.ArrayList<>())
  ...
  @Override
  public VoiceLessonResponseDTO updateLesson(
      String id,
      String title,
      String content,
      VoiceLessonCategory category,
    ...
    if (audioFile == null || audioFile.isEmpty()) {
      ...
      try {
        ...
        throw new AppException(
            ...
            "Failed to upload sample audio: " + SecurityUtils.safeMessage(e));
      }
    }
    return lessonMapper.toResponseDTO(lessonRepository.save(lesson));
  }
  ...
  @Override
  public List<VoiceLessonResponseDTO> searchLessons(
      String searchTerm, VoiceLessonCategory category) {
  ...
  @Override
  public PracticeSessionResponseDTO analyzePractice(
      String lessonId, String userId, MultipartFile audioFile) {
    ...
    if (plan == SubscriptionPlan.FREE) {
      ...
      if (count >= PlanConfig.FREE_SESSION_LIMIT) {
        throw new AppException(
            ...
            "Free plan limit: "
                + PlanConfig.FREE_SESSION_LIMIT
                + " sessions. Upgrade to continue.");
      }
    } else if (plan == SubscriptionPlan.DAILY) {
      if (user.getAiSessionsUsed() >= PlanConfig.DAILY_AI_SESSION_LIMIT) {
        throw new AppException(
            ErrorCode.LIMIT_EXCEEDED,
            "Goi Ngay: da dung het "
                + PlanConfig.DAILY_AI_SESSION_LIMIT
                + " AI sessions. Gia han them 1 ngay de tiep tuc.");
      }
      ...
      if (user.getAiSessionsUsed() >= PlanConfig.BASIC_AI_SESSION_LIMIT) {
        throw new AppException(
            ErrorCode.LIMIT_EXCEEDED,
            "BASIC plan limit: "
                + PlanConfig.BASIC_AI_SESSION_LIMIT
                + " AI coaching sessions/month. Upgrade to FULL for unlimited.");
      }
    }
    ...
    try {
      body.add(
          ...
          objectMapper.writeValueAsString(lesson.getEvaluationCriteria()));
    } catch (Exception e) {
    ...
    try {
      Map<String, Object> response =
          restTemplate.postForObject(aiServiceUrl, requestEntity, Map.class);
      ...
      List<PracticeSession.ExpertTip> expertTipsVi =
          tipsViRaw.stream()
              .map(
                  t ->
                      PracticeSession.ExpertTip.builder()
                          .label(t.get("label"))
                          .tip(t.get("tip"))
                          .build())
              .collect(Collectors.toList());

      List<PracticeSession.ExpertTip> expertTipsEn =
          tipsEnRaw.stream()
              .map(
                  t ->
                      PracticeSession.ExpertTip.builder()
      ...
      double overallScore =
          response.get("overall_score") != null
              ? ((Number) response.get("overall_score")).doubleValue()
              : 0.0;

      double cerRate =
          response.get("cer_rate") != null
              ? ((Number) response.get("cer_rate")).doubleValue()
              : 0.0;
      double werRate =
          response.get("wer_rate") != null
              ? ((Number) response.get("wer_rate")).doubleValue()
              : 0.0;

      @SuppressWarnings("unchecked")
      Map<String, Object> analysisMeta = (Map<String, Object>) response.get("analysis_meta");
      double durationSeconds =
          analysisMeta != null && analysisMeta.get("duration_total_sec") != null
              ? ((Number) analysisMeta.get("duration_total_sec")).doubleValue()
              : 0.0;

      @SuppressWarnings("unchecked")
      Map<String, Object> spectralFeatures =
          (Map<String, Object>) response.get("spectral_features");
      @SuppressWarnings("unchecked")
      ...
      @SuppressWarnings("unchecked")
      Map<String, Object> emotionBreakdown =
          (Map<String, Object>) response.get("emotion_breakdown");
      @SuppressWarnings("unchecked")
      List<Map<String, Object>> wordAlignment =
          (List<Map<String, Object>>) response.get("word_alignment");
      @SuppressWarnings("unchecked")
      List<Map<String, Object>> sentenceFeedback =
          (List<Map<String, Object>>) response.get("sentence_feedback");
      ...
      try {
        gamificationService.processPracticeSession(
            userId,
            lessonId,
            session.getAccuracyScore(),
      ...
      throw new AppException(
          ErrorCode.INTERNAL_ERROR, "Error calling AI service: " + e.getMessage());
    }
  }
        ...
        .map(
            s -> {
              ...
              if (lesson != null) {
                ...
                dto.setLessonCategory(
                    lesson.getCategory() != null ? lesson.getCategory().name() : null);
                dto.setLessonDifficulty(lesson.getDifficulty());
                dto.setLessonDescription(lesson.getDescription());
              }
              log.info(
                  ">>> Mapping Session {} -> Lesson {} -> Title: {}",
                  s.getId(),
                  s.getLessonId(),
                  dto.getLessonTitle());
              return dto;
  ...
  public List<PracticeSessionResponseDTO> getUserLessonHistory(String userId, String lessonId) {
    List<PracticeSession> sessions =
        sessionRepository.findByUserIdAndLessonId(userId, lessonId).stream()
  ...
  public PracticeSessionResponseDTO getPracticeSessionById(String id) {
    PracticeSession session =
        sessionRepository
            .findById(id)
            .orElseThrow(
                () -> new AppException(ErrorCode.USER_NOT_FOUND, "Practice session not found"));

    PracticeSessionResponseDTO dto = sessionMapper.toResponseDTO(session);
    lessonRepository
        .findById(session.getLessonId())
    ...
    try {
      org.springframework.http.ResponseEntity<byte[]> response =
          restTemplate.exchange(
              ttsServiceUrl, org.springframework.http.HttpMethod.POST, requestEntity, byte[].class);
      byte[] wav = response.getBody();
      return wav != null ? wav : new byte[0];
    } catch (Exception e) {
      log.error("Error calling TTS stream service", e);
      throw new AppException(
          ErrorCode.INTERNAL_ERROR, "TTS service error: " + SecurityUtils.safeMessage(e));
    }
  }
  ...
  public Object proxyAnalyzeVoice(MultipartFile audioFile, String scriptOrigin) {
    ...
    body.add(
        "script_origin", scriptOrigin != null && !scriptOrigin.isBlank() ? scriptOrigin : "N/A");
