  ...
  public GoogleIdentity verify(@NonNull String idToken) {
    if (verifier == null) {
      throw new AppException(
          ErrorCode.GOOGLE_TOKEN_INVALID, "Google Sign-In is not configured on this server.");
    }
    try {
      ...
      if (email == null || email.isBlank()) {
        throw new AppException(
            ErrorCode.GOOGLE_TOKEN_INVALID, "Google token did not include an email.");
      }
      if (!emailVerified) {
        throw new AppException(
            ErrorCode.GOOGLE_TOKEN_INVALID, "Google account email is not verified.");
      }
