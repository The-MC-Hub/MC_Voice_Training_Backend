...
public class GamificationServiceImpl implements GamificationService {
  ...
  private static final double PRACTICE_SESSION_ESTIMATED_HOURS =
      0.05; // 3 minutes per session — fallback only when AI didn't report a duration
  ...
  @Override
  public UserStats processPracticeSession(
      String userId,
      String lessonId,
      double accuracy,
      double rhythm,
    ...
    log.info(
        "Processing gamification for user: {} with accuracy: {} and rhythm: {}",
        userId,
        accuracy,
        rhythm);
    ...
    // fall back to the flat estimate only if the AI service didn't report one (e.g. old sessions,
    // proxy path).
    double hoursToAdd =
        durationSeconds > 0 ? durationSeconds / 3600.0 : PRACTICE_SESSION_ESTIMATED_HOURS;
    ...
    if (overallScore >= 90.0) {
      ...
      stats.setLongestHighScoreStreak(
          Math.max(stats.getLongestHighScoreStreak(), stats.getHighScoreStreak()));
    } else {
  ...
  public UserStats processLoginStreak(String userId) {
    ...
    if (lastFreeze == null
        || lastFreeze.getMonth() != today.getMonth()
        || lastFreeze.getYear() != today.getYear()) {
  ...
  private String issueCourseCompletionVoucher(String userId, String courseId) {
    String code =
        "COURSE30-"
            + courseId.substring(Math.max(0, courseId.length() - 6)).toUpperCase()
            + "-"
  ...
  private void processCompetitions(
      String userId, String lessonId, double accuracy, double rhythm, double xpEarned) {
    ...
    for (Competition comp : activeCompetitions) {
      ...
      if (comp.getChallengeScriptId() != null && comp.getChallengeScriptId().equals(lessonId)) {
        Optional<CompetitionRecord> recordOpt =
            competitionRecordRepository.findByCompetitionIdAndUserId(comp.getId(), userId);
