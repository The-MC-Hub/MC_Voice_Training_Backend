...
public class CourseServiceImpl implements CourseService {
  ...
  public List<String> getAllCourseTypes() {
    return Arrays.stream(CourseType.values()).map(Enum::name).collect(Collectors.toList());
  }
  ...
  private List<CourseResponseDTO> mapWithProgress(List<Course> courses, String userId) {
    ...
    java.util.Map<String, CourseEnrollment> enrollmentMap =
    ...
    return courses.stream()
        .map(
            c ->
                toSummaryDTO(
                    c,
                    enrollmentMap.containsKey(c.getId())
                        ? toProgressDTO(enrollmentMap.get(c.getId()))
                        : null))
  ...
  public List<CourseResponseDTO> getMilestoneCourses(String userId) {
    ...
    List<Course> milestones =
        courseRepository
            .findByLearningPathTypeAndIsActiveTrue(LearningPathType.MILESTONE_PATH)
            .stream()
            .sorted(
    ...
    for (Course c : milestones) {
      CourseResponseDTO.EnrollmentProgressDTO progress =
          userId != null
              ? enrollmentRepository
                  .findByUserIdAndCourseId(userId, c.getId())
  ...
  public CourseResponseDTO getCourseDetail(String courseId, String userId) {
    ...
    if (userId != null) {
      ...
      boolean purchased =
          user != null
              && user.getPurchasedCourseIds() != null
  ...
  public ReadingGuide getReadingGuide(String id) {
    return readingGuideRepository
        .findById(id)
        .orElseThrow(
            () -> new AppException(ErrorCode.COURSE_NOT_FOUND, "Reading Guide not found: " + id));
  }
  ...
  public CourseResponseDTO.EnrollmentProgressDTO enroll(String courseId, String userId) {
    ...
    if (!hasCourseAccess(courseId, userId)) {
      throw new AppException(
          ErrorCode.COURSE_REQUIRES_PLAN,
          "Course requires BASIC plan or higher, or individual purchase");
    }
    CourseEnrollment enrollment =
        CourseEnrollment.builder().userId(userId).courseId(courseId).build();
    ...
    if (enrollmentRepository.existsByUserIdAndCourseId(userId, courseId)) {
      return toProgressDTO(
          enrollmentRepository
              .findByUserIdAndCourseId(userId, courseId)
              .orElseThrow(
                  () -> new AppException(ErrorCode.ENROLLMENT_NOT_FOUND, "Enrollment not found")));
    }
    CourseEnrollment enrollment =
        CourseEnrollment.builder().userId(userId).courseId(courseId).build();
    return toProgressDTO(enrollmentRepository.save(enrollment));
  ...
  @Override
  public CourseResponseDTO.EnrollmentProgressDTO completeLesson(
      String courseId, String lessonId, String userId) {
    if (!hasCourseAccess(courseId, userId)) {
      throw new AppException(
          ErrorCode.COURSE_REQUIRES_PLAN, "Plan expired. Renew to continue learning.");
    }
    Course course = findCourse(courseId);
    if (!course.getLessonIds().contains(lessonId)) {
      throw new AppException(
          ErrorCode.VALIDATION_FAILED, "Lesson does not belong to this course: " + lessonId);
    }
  ...
  @Override
  public CourseResponseDTO.EnrollmentProgressDTO completeReading(
    ...
    if (!course.getReadingIds().contains(readingId)) {
      throw new AppException(
          ErrorCode.VALIDATION_FAILED, "Reading does not belong to this course: " + readingId);
    }
    CourseEnrollment enrollment = findEnrollment(userId, courseId);
  ...
  @Override
  public ExerciseResultDTO completeExercise(
            ...
            .orElseThrow(
                () ->
                    new AppException(
                        ...
                        "Exercise does not belong to this course: " + exerciseId));
  ...
  public QuizResultDTO submitQuiz(String courseId, String userId, QuizSubmitRequest request) {
    ...
    if (request.getAnswers().size() != questions.size()) {
      throw new AppException(
          ErrorCode.QUIZ_ANSWER_MISMATCH,
    ...
    for (int i = 0; i < questions.size(); i++) {
      ...
      feedback.add(
          QuizResultDTO.QuestionFeedback.builder()
              .questionIndex(i)
    ...
    // Award XP + voucher on first-time full course completion — best-effort, never fails the quiz
    // submission
    ...
    if (courseCompletedNow) {
      try {
        ...
        log.error(
            "Failed to process gamification for course completion: user={} course={}",
            userId,
            courseId,
            e);
      }
  ...
  public CourseProgressStatsDTO getCourseProgressStats(String courseId, String userId) {
    ...
    List<PracticeSession> sessions =
        practiceSessionRepository.findByUserId(userId).stream()
            .filter(s -> lessonIdSet.contains(s.getLessonId()))
            .collect(Collectors.toList());

    double totalHours =
        sessions.stream().mapToDouble(PracticeSession::getDurationSeconds).sum() / 3600.0;
    double avgScore =
        sessions.stream().mapToDouble(PracticeSession::getOverallScore).average().orElse(0.0);

    java.time.ZoneId zone = java.time.ZoneId.systemDefault();
    Map<String, List<PracticeSession>> byDate =
        sessions.stream()
            .collect(
                Collectors.groupingBy(s -> s.getCreatedAt().atZone(zone).toLocalDate().toString()));
    List<CourseProgressStatsDTO.ScorePoint> scoreOverTime =
        byDate.entrySet().stream()
            .map(
                en ->
                    CourseProgressStatsDTO.ScorePoint.builder()
                        .date(en.getKey())
                        .avgScore(
                            en.getValue().stream()
    ...
    Map<String, List<PracticeSession>> byLesson =
        sessions.stream().collect(Collectors.groupingBy(PracticeSession::getLessonId));
    Map<String, VoiceLesson> lessonMap =
        lessonRepository.findAllById(byLesson.keySet()).stream()
            .collect(Collectors.toMap(VoiceLesson::getId, l -> l));
    List<CourseProgressStatsDTO.WeakLesson> weakest =
    ...
    return courseRepository.findAll().stream()
        .map(
            c -> {
              ...
              dto.setTotalCompletions(
                  enrollmentRepository.countByCourseIdAndCompletedTrue(c.getId()));
              return dto;
            })
        .collect(Collectors.toList());
  }
  ...
  @Override
  public CourseResponseDTO updatePricing(
      String courseId, Integer priceVnd, Integer discountPercent) {
    Course course = findCourse(courseId);
    if (priceVnd != null) {
      if (priceVnd < 0)
        throw new AppException(ErrorCode.VALIDATION_FAILED, "priceVnd must be >= 0");
  ...
  public boolean hasCourseAccess(String courseId, String userId) {
    ...
    boolean isPaidPlan =
        plan == com.mchub.enums.SubscriptionPlan.DAILY
  ...
  private CourseEnrollment findEnrollment(String userId, String courseId) {
    return enrollmentRepository
        ...
        .orElseThrow(
            () ->
                new AppException(
                    ErrorCode.ENROLLMENT_NOT_FOUND, "Not enrolled in course: " + courseId));
  }

  private void recalcCompletion(CourseEnrollment e, Course c) {
    int total =
        c.getLessonIds().size()
            + c.getReadingIds().size()
            + c.getExercises().size()
            + 1; // +1 for quiz
    int done =
        e.getCompletedLessonIds().size()
    ...
    boolean allExercises =
        e.getCompletedExerciseIds()
            .containsAll(
  ...
  private Course fromRequest(SaveCourseRequest req, Course target) {
    ...
    target.setCaseStudyIds(
        req.getCaseStudyIds() != null ? req.getCaseStudyIds() : new ArrayList<>());
    ...
    if (req.getQuizQuestions() != null) {
      target.setQuizQuestions(
          req.getQuizQuestions().stream()
              .map(
                  q ->
    ...
    if (req.getExercises() != null) {
      target.setExercises(
          req.getExercises().stream()
              .map(
                  ex ->
                      Course.Exercise.builder()
                          .id(
                              ex.getId() != null
                                  ? ex.getId()
                          ...
                          .distractors(
                              ex.getDistractors() != null ? ex.getDistractors() : new ArrayList<>())
  ...
  private CourseResponseDTO toSummaryDTO(
      Course c, CourseResponseDTO.EnrollmentProgressDTO progress) {
  ...
  private CourseResponseDTO toDetailDTO(
      Course c, CourseResponseDTO.EnrollmentProgressDTO progress) {
    CourseResponseDTO dto = toSummaryDTO(c, progress);
    ...
    if (!c.getLessonIds().isEmpty()) {
      ...
      dto.setLessons(
          lessons.stream()
              .map(
                  l ->
                      VoiceLessonResponseDTO.builder()
                          .id(l.getId())
                          .title(l.getTitle())
    ...
    if (!c.getReadingIds().isEmpty()) {
      ...
      dto.setReadings(
          readings.stream()
              .map(
                  r ->
                      CourseResponseDTO.ReadingGuideDTO.builder()
                          .id(r.getId())
                          .title(r.getTitle())
    ...
    dto.setQuizQuestions(
        c.getQuizQuestions().stream()
            .map(
                q ->
                    CourseResponseDTO.QuizQuestionDTO.builder()
                        .question(q.getQuestion())
                        .options(q.getOptions())
                        .category(q.getCategory())
                        .build())
    ...
    if (!c.getCaseStudyIds().isEmpty()) {
      dto.setCaseStudies(
          caseStudyRepository.findAllById(c.getCaseStudyIds()).stream()
              .map(
                  cs ->
                      CourseResponseDTO.CaseStudySummaryDTO.builder()
                          .id(cs.getId())
                          .title(cs.getTitle())
