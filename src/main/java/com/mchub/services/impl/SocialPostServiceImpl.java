...
public class SocialPostServiceImpl implements SocialPostService {
  ...
  public List<SocialPostDTO> getActivePosts() {
    return socialPostRepository.findByActiveTrueOrderBySortOrderDesc().stream()
        .map(this::toDTO)
  ...
  public List<SocialPostDTO> getAllPostsAdmin() {
    return socialPostRepository.findAllByOrderBySortOrderDesc().stream()
        .map(this::toDTO)
        .collect(Collectors.toList());
  }
  ...
  public SocialPostDTO updatePost(String id, SaveSocialPostRequest request) {
    SocialPost post =
        EntityUtils.getOrThrow(
            socialPostRepository, id, ErrorCode.RESOURCE_NOT_FOUND, "Social post not found: " + id);
