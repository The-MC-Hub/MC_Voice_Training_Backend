...
public class CommunityServiceImpl implements CommunityService {
  ...
  public CommunityStatsDTO getCommunityStats() {
    ...
    double totalPracticeHours =
        userStatsRepository.findAll().stream().mapToDouble(UserStats::getTotalPracticeHours).sum();
    ...
    if (isWeekly) {
      ...
      all =
          switch (type.toLowerCase()) {
            case "streak" ->
                userStatsRepository
                    .findAllByOrderByCurrentStreakDesc(Pageable.unpaged())
                    .getContent();
            case "precision" ->
                userStatsRepository
                    .findAllByOrderByCumulativeXPDesc(Pageable.unpaged())
                    .getContent();
            case "sessions" ->
                userStatsRepository
                    .findAllByOrderByTotalSessionsDesc(Pageable.unpaged())
                    .getContent();
            default ->
                userStatsRepository
                    .findAllByOrderByTotalPracticeHoursDesc(Pageable.unpaged())
                    .getContent();
          };
    }
  ...
  public List<ActiveArenaResponseDTO> getActiveArenas(String userId) {
    ...
    for (Competition comp : activeComps) {
      ...
      if (comp.getChallengeScriptId() != null) {
        Optional<VoiceLesson> lessonOpt =
            voiceLessonRepository.findById(comp.getChallengeScriptId());
      ...
      List<CompetitionRecord> records =
          competitionRecordRepository.findByCompetitionId(comp.getId());
      List<ArenaLeaderboardEntryDTO> leaderboard =
          records.stream()
              .sorted(
                  (r1, r2) ->
                      Double.compare(
              ...
              .map(
                  r ->
                      ArenaLeaderboardEntryDTO.builder()
                          .userId(r.getUserId())
      ...
      if (userId != null) {
        userRecord =
            leaderboard.stream().filter(e -> e.getUserId().equals(userId)).findFirst().orElse(null);
      }

      arenas.add(
          ActiveArenaResponseDTO.builder()
              .competition(comp)
