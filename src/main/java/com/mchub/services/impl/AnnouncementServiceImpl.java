...
public class AnnouncementServiceImpl implements AnnouncementService {
  ...
  public Announcement getById(String id) {
    return EntityUtils.getOrThrow(
        announcementRepo, id, ErrorCode.RESOURCE_NOT_FOUND, "Announcement not found: " + id);
  }
  ...
  public Announcement createFromTrigger(
    ...
    Announcement draft =
        Announcement.builder()
            ...
            .emailDesign(
                Announcement.EmailDesignData.builder()
                    .title(emailTitle)
                    .description(content)
  ...
  @Override
  @Async
  public void triggerNewLessonNotification(
      String lessonId, String lessonTitle, String lessonCategory) {
    try {
      Announcement.TargetCriteria criteria = new Announcement.TargetCriteria();
      List<User> recipients = resolveTargetUsers(criteria);
      String title = "Bài học mới: " + lessonTitle;
      String message =
          "Bài học mới thuộc danh mục "
              + lessonCategory
              + " vừa được xuất bản! Tham gia luyện tập ngay.";
      String actionUrl = "/m/courses";

      for (User u : recipients) {
        try {
          notificationService.notify(
              u.getId(), NotificationType.ANNOUNCEMENT, title, message, actionUrl, false);
        } catch (Exception e) {
  ...
  public void triggerDiscountNotification(String code, int discountPercent, String expiresAtStr) {
    try {
      ...
      String message =
          "Nhập mã "
              + code
              + " khi thanh toán để nhận ngay ưu đãi "
              + discountPercent
              + "%. Hạn dùng: "
              + expiresAtStr;
  ...
  public String renderEmailPreview(String id) {
    Announcement announcement = getById(id);
    return emailService.buildHtmlEmail(
        "bạn",
        announcement.getContent(),
        announcement.getType() != null ? announcement.getType().name() : null);
  }
  ...
  public Map<String, Object> previewStats(String id) {
    ...
    return Map.of(
        ...
        "channel",
            announcement.getChannel() != null
                ? announcement.getChannel()
                : Announcement.ChannelType.BOTH,
        "status",
    ...
    if (overrideRecipientIds != null && !overrideRecipientIds.isEmpty()) {
      recipients =
          userRepository.findAll().stream()
              ...
              .toList();
    } else if (announcement.getRecipientIds() != null
        && !announcement.getRecipientIds().isEmpty()) {
      recipients =
          userRepository.findAll().stream()
    ...
    return all.stream()
        .filter(
            u -> {
              ...
              if (c.getRegisteredAfter() != null
                  && (u.getCreatedAt() == null
                      || u.getCreatedAt().isBefore(c.getRegisteredAfter()))) return false;
              if (c.getRegisteredBefore() != null
                  && (u.getCreatedAt() == null
                      || u.getCreatedAt().isAfter(c.getRegisteredBefore()))) return false;
              return true;
            })
        .toList();
  }
  ...
  private void dispatchAnnouncement(Announcement a, List<User> recipients) {
    boolean sendInApp =
        a.getChannel() == Announcement.ChannelType.IN_APP
            || a.getChannel() == Announcement.ChannelType.BOTH;
    boolean sendEmail =
        a.getChannel() == Announcement.ChannelType.EMAIL
            || a.getChannel() == Announcement.ChannelType.BOTH;
