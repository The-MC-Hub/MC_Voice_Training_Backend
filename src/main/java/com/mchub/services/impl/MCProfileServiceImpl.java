...
public class MCProfileServiceImpl implements MCProfileService {
  ...
  public Map<String, Object> getDashboardStats(String userId) {
    CompletableFuture<Long> totalPracticesFuture =
        CompletableFuture.supplyAsync(() -> practiceSessionRepository.countByUserId(userId));

    // Single fetch reused for both averages — avoids a duplicate DB round-trip
    CompletableFuture<Map<String, Double>> averagesFuture =
        CompletableFuture.supplyAsync(
            () -> {
              var sessions = practiceSessionRepository.findByUserId(userId);
              double avgAccuracy =
                  sessions.stream().mapToDouble(s -> s.getAccuracyScore()).average().orElse(0.0);
              double avgWpm =
                  sessions.stream().mapToDouble(s -> s.getSpeakingRateWpm()).average().orElse(0.0);
  ...
  public MCProfile updateProfile(String userId, MCProfile profileData) {
    MCProfile existing = mcProfileRepository.findByUser(userId).orElse(new MCProfile());
