...
public class RecommendationServiceImpl implements RecommendationService {
  ...
  public void notifyMatchingClients(MCProfile profile) {
    if (profile == null || profile.getStatus() != MCStatus.AVAILABLE || profile.getUser() == null)
      return;
  ...
  public void runScheduledSweep() {
    List<MCProfile> availableProfiles =
        mcProfileRepository.findAll().stream()
            .filter(p -> p.getStatus() == MCStatus.AVAILABLE)
            .toList();

    log.info(
        "🔍 [Recommendation] Scheduled sweep: {} available MC profiles", availableProfiles.size());
  ...
  private double scoreMatch(MCProfile p, SearchInterest interest) {
    ...
    if (interest.getEventTypes() != null
        && !interest.getEventTypes().isEmpty()
        && p.getEventTypes() != null) {
      long matchCount =
          p.getEventTypes().stream().filter(interest.getEventTypes()::contains).count();
      score += Math.min(matchCount * 1.5, 3.0);
    }

    if (interest.getRegions() != null
        && !interest.getRegions().isEmpty()
        && p.getRegions() != null) {
      boolean regionMatch = p.getRegions().stream().anyMatch(interest.getRegions()::contains);
      if (regionMatch) score += 2.0;
    }

    if (interest.getBudgetMax() != null
        && interest.getBudgetMax() > 0
        && p.getRates() != null
        && p.getRates().getMax() > 0
        && p.getRates().getMax() <= interest.getBudgetMax()) {
      score += 2.0;
    }

    if (interest.getMinExperience() != null
        && interest.getMinExperience() > 0
