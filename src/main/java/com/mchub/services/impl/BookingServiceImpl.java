...
public class BookingServiceImpl implements BookingService {
  ...
  private static final Set<BookingStatus> VALID_FROM_PENDING =
      Set.of(BookingStatus.ACCEPTED, BookingStatus.REJECTED);
  private static final Set<BookingStatus> VALID_FROM_ACCEPTED =
      Set.of(BookingStatus.PAID, BookingStatus.CANCELLED);
  private static final Set<BookingStatus> VALID_FROM_PAID =
  ...
  public Booking createBooking(CreateBookingRequest req, String clientId) {
    boolean available =
        checkAvailabilityParallel(
            req.getMc(), req.getEventDate(), req.getStartTime(), req.getEndTime());
    if (!available) {
  ...
  private boolean checkAvailabilityParallel(
      String mcId, java.time.LocalDate date, String start, String end) {
    CompletableFuture<Boolean> acceptedCheck =
        CompletableFuture.supplyAsync(
            () ->
                bookingRepository.findByMcAndStatus(mcId, BookingStatus.ACCEPTED).stream()
                    .noneMatch(b -> isOverlapping(b, date, start, end)));
    CompletableFuture<Boolean> paidCheck =
        CompletableFuture.supplyAsync(
            () ->
                bookingRepository.findByMcAndStatus(mcId, BookingStatus.PAID).stream()
                    .noneMatch(b -> isOverlapping(b, date, start, end)));
