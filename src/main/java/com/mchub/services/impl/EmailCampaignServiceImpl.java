...
public class EmailCampaignServiceImpl implements EmailCampaignService {
  ...
  private String generateHtmlFromDesign(EmailTemplate.DesignData d) {
    String logoHtml =
        (d.getLogoUrl() != null && !d.getLogoUrl().isBlank())
            ? "<img src=\""
                + d.getLogoUrl()
                + "\" alt=\"Logo\" style=\"max-height:50px;display:block;margin:0 auto 20px auto;\"/>"
            : "";
    String bannerHtml =
        (d.getBannerUrl() != null && !d.getBannerUrl().isBlank())
            ? "<img src=\""
                + d.getBannerUrl()
                + "\" alt=\"Banner\" style=\"width:100%;max-width:600px;height:auto;display:block;border-radius:8px;margin-bottom:24px;\"/>"
            : "";
    String buttonHtml =
  ...
  private String replacePlaceholders(String content, String name, String email) {
    ...
    result =
        result.replaceAll(
            "\\{\\{\\s*(tên|họ tên|họ và tên|ten|hoten|fullname|full name|recipient)\\s*\\}\\}",
            name != null ? name : "");
    result =
        result.replaceAll(
            "\\{\\{\\s*(email|địa chỉ email|e-mail)\\s*\\}\\}", email != null ? email : "");
  ...
  @Override
  public EmailTemplateDTO createTemplate(
      String name, String subject, String htmlContent, EmailTemplate.DesignData designData) {
    String html =
        (htmlContent != null && !htmlContent.isBlank())
            ? htmlContent
            : generateHtmlFromDesign(
                designData != null ? designData : EmailTemplate.DesignData.builder().build());
  ...
  public EmailTemplateDTO updateTemplate(
      ...
      String subject,
      String htmlContent,
      EmailTemplate.DesignData designData) {
    EmailTemplate template =
        templateRepository
            .findById(id)
            .orElseThrow(
                () -> new AppException(ErrorCode.RESOURCE_NOT_FOUND, "Template not found: " + id));
  ...
  @Override
  public List<UserPreviewDTO> previewRecipients(
      String targetType,
      List<String> targetPlans,
      List<String> targetRoles,
      List<String> targetEmails) {
  ...
  @Override
  public EmailCampaignResponseDTO sendCampaign(
      String templateId,
      String subject,
      String targetType,
      List<String> targetPlans,
  ...
  public void processCampaignAsync(String campaignId) {
    ...
    campaignRepository.save(campaign);
    log.info(
        "[Campaign {}] Completed. Success: {}, Failed: {}", campaignId, successCount, failedCount);
  }
  ...
  public EmailCampaignResponseDTO getCampaignById(String id) {
    return toCampaignDTO(
        campaignRepository
            .findById(id)
            .orElseThrow(
                () -> new AppException(ErrorCode.RESOURCE_NOT_FOUND, "Campaign not found: " + id)));
  }
  ...
  private EmailCampaignResponseDTO toCampaignDTO(EmailCampaign c) {
    return EmailCampaignResponseDTO.builder()
        ...
        .targetPlans(c.getTargetPlans())
        .targetRoles(c.getTargetRoles())
        .targetEmails(c.getTargetEmails())
        .build();
  }
