...
public class PublicServiceImpl implements PublicService {
  ...
  public List<MCTrainingStatsDTO> getFeaturedMCTrainingStats() {
    ...
    List<String> userIds =
        profiles.stream().map(MCProfile::getUser).filter(Objects::nonNull).toList();
    ...
    Map<String, List<com.mchub.models.PracticeSession>> sessionsByUser =
        practiceSessionRepository.findByUserIdIn(userIds).stream()
            .collect(
                java.util.stream.Collectors.groupingBy(
                    com.mchub.models.PracticeSession::getUserId));
    ...
    for (MCProfile profile : profiles) {
      ...
      double avgAcc =
          sessions.stream()
              .mapToDouble(com.mchub.models.PracticeSession::getAccuracyScore)
              .average()
              .orElse(0.0);
      double avgRhy =
          sessions.stream()
              .mapToDouble(com.mchub.models.PracticeSession::getRhythmScore)
              .average()
              .orElse(0.0);

      statsList.add(
          MCTrainingStatsDTO.builder()
              .mcId(profile.getId())
              .userId(uId)
  ...
  public Map<String, Object> getLandingData() {
    ...
    CompletableFuture<Long> professionalsCount =
        CompletableFuture.supplyAsync(() -> userRepository.countByRole(UserRole.MC));
  ...
  public List<MCProfileResponseDTO> discoverMCs(String category) {
    ...
    List<String> userIds =
        profiles.stream().map(MCProfile::getUser).filter(Objects::nonNull).distinct().toList();
  ...
  private void applyFieldVisibility(MCProfileResponseDTO dto) {
    ...
    if (Boolean.FALSE.equals(visibility.get("rates"))) {
      dto.setRatesMin(null);
      dto.setRatesMax(null);
    }
  ...
  public List<MCSearchResultDTO> searchMCs(SearchMCRequest req) {
    ...
    return profiles.stream()
        ...
        .map(
            p -> {
              ...
              if (req.getKeyword() != null && !req.getKeyword().isBlank()) {
                ...
                boolean nameMatch =
                    u != null && u.getName() != null && u.getName().toLowerCase().contains(kw);
                boolean styleMatch =
                    p.getStyles() != null
                        && p.getStyles().stream().anyMatch(s -> s.toLowerCase().contains(kw));
                boolean bioMatch =
                    p.getBiography() != null && p.getBiography().toLowerCase().contains(kw);
                boolean langMatch =
                    p.getLanguages() != null
                        && p.getLanguages().stream().anyMatch(l -> l.toLowerCase().contains(kw));
                boolean notableMatch =
                    p.getNotableEvents() != null
                        && p.getNotableEvents().stream()
                            .anyMatch(n -> n.toLowerCase().contains(kw));
                boolean personalityMatch =
                    (p.getPersonality() != null && p.getPersonality().toLowerCase().contains(kw))
                        || (p.getHostingStyle() != null
                            && p.getHostingStyle().toLowerCase().contains(kw));
                if (nameMatch
                    || styleMatch
                    || bioMatch
                    || langMatch
              ...
              if (req.getRegions() != null && !req.getRegions().isEmpty()) {
                if (p.getRegions() != null && !p.getRegions().isEmpty()) {
                  long regionMatchCount =
                      p.getRegions().stream().filter(r -> req.getRegions().contains(r)).count();
              ...
              if (req.getStyles() != null && !req.getStyles().isEmpty()) {
                if (p.getStyles() != null && !p.getStyles().isEmpty()) {
                  long styleMatchCount =
                      p.getStyles().stream().filter(s -> req.getStyles().contains(s)).count();
              ...
              if (req.getLanguages() != null && !req.getLanguages().isEmpty()) {
                if (p.getLanguages() != null && !p.getLanguages().isEmpty()) {
                  long langMatchCount =
                      p.getLanguages().stream().filter(l -> req.getLanguages().contains(l)).count();
              ...
              if (req.getHostingStyle() != null && !req.getHostingStyle().isBlank()) {
                if (p.getHostingStyle() != null
                    && p.getHostingStyle().equalsIgnoreCase(req.getHostingStyle())) {
                  score += 1.5;
              ...
              if (p.getRates() != null) {
                ...
                if (req.getBudgetMax() != null
                    && req.getBudgetMax() > 0
                    && p.getRates().getMax() <= req.getBudgetMax()) {
                  budgetScore += 1.5;
                }
                if (req.getBudgetMin() != null
                    && req.getBudgetMin() > 0
                    && p.getRates().getMin() >= req.getBudgetMin()) {
                  budgetScore += 1.0;
                }
                if (budgetScore >= 2.5
                    && req.getBudgetMax() != null
                    && req.getBudgetMax() > 0
                    && p.getRates().getMax() > 0) {
              ...
              if (req.getMinExperience() != null
                  && req.getMinExperience() > 0
                  && p.getExperience() >= req.getMinExperience()) {
              ...
              if (p.getLastActive() != null) {
                long daysSinceActive =
                    java.time.Duration.between(p.getLastActive(), LocalDateTime.now()).toDays();
        ...
        .sorted(
            (a, b) -> {
              if ("rating".equals(req.getSortBy()))
                return Double.compare(b.getProfile().getRating(), a.getProfile().getRating());
              if ("experience".equals(req.getSortBy()))
                return Integer.compare(
                    b.getProfile().getExperience(), a.getProfile().getExperience());
              if ("price_low".equals(req.getSortBy()))
                return Double.compare(
                    a.getProfile().getRatesMin() != null ? a.getProfile().getRatesMin() : 0,
                    b.getProfile().getRatesMin() != null ? b.getProfile().getRatesMin() : 0);
              if ("price_high".equals(req.getSortBy()))
                return Double.compare(
  ...
  private void saveSearchInterest(String clientId, SearchMCRequest req) {
    try {
      ...
      SearchInterest interest =
          searchInterestRepository
              .findByClientIdAndKeyword(clientId, key)
              .orElse(
                  SearchInterest.builder()
                      .clientId(clientId)
                      .keyword(key)
  ...
  @Override
  public List<EnumOptionDTO> getUserRoles() {
    return toEnumOptions(UserRole.class);
  }

  @Override
  public List<EnumOptionDTO> getReportReasons() {
    return toEnumOptions(ReportReason.class);
  }
