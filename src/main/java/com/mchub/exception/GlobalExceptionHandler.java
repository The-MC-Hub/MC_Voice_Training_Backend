...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ApiResponse<Void>> handleAppException(AppException ex) {
    ...
    log.error(
        "🔴 [{}] {} | HTTP {}", code.getCode(), ex.getMessage(), code.getHttpStatus().value());
    if (ex.getCause() != null) {
      log.error("   └─ Cause: {}", ex.getCause().getMessage());
    }
    return ResponseEntity.status(Objects.requireNonNull(code.getHttpStatus()))
  ...
  public ResponseEntity<ApiResponse<Void>> handleAccessDeniedException(AccessDeniedException ex) {
    ...
    return ResponseEntity.status(HttpStatus.FORBIDDEN)
        .body(
            buildErrorResponse(
                ErrorCode.ACCESS_DENIED.getCode(), ErrorCode.ACCESS_DENIED.getDefaultMessage()));
  }
  ...
  public ResponseEntity<ApiResponse<Void>> handleIllegalStateException(IllegalStateException ex) {
    ...
    return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
        .body(
            buildErrorResponse(
                ErrorCode.USER_NOT_AUTHENTICATED.getCode(),
  ...
  public ResponseEntity<ApiResponse<Void>> handleUnreadableBody(
    ...
    return ResponseEntity.status(HttpStatus.BAD_REQUEST)
        .body(
            buildErrorResponse(
                ErrorCode.VALIDATION_FAILED.getCode(), "Dữ liệu gửi lên không đúng định dạng"));
  }
  ...
  public ResponseEntity<ApiResponse<Void>> handleMethodNotAllowed(
    ...
    log.warn(
        "🔵 [{}] Method not allowed: {}", ErrorCode.METHOD_NOT_ALLOWED.getCode(), ex.getMessage());
