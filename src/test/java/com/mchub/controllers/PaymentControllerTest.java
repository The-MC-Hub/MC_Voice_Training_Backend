...
/**
 * @WebMvcTest for PaymentController. SecurityUtils.getCurrentUserId() is exercised with a real
 * Authentication populated in SecurityContextHolder per-test (security filter chain itself is
 * disabled via addFilters=false — /simulate-success and /admin/complete/** have @PreAuthorize but
 * method security is not active in a pure @WebMvcTest slice without @EnableMethodSecurity context,
 * so those endpoints are reachable here regardless of role; that authorization boundary is covered
 * instead by DEFECT-001-style whitelist checks and manual UC-06 system testing).
 */
...
class PaymentControllerTest {
  ...
  class PublicEndpoints {
    ...
    void returnsActivePlans() throws Exception {
      when(planService.getActivePlans())
          .thenReturn(List.of(PlanDefinition.builder().id("p1").build()));

      mockMvc
          .perform(get("/api/v1/payment/plans"))
    ...
    void returnsFlashDeals() throws Exception {
      ...
      mockMvc.perform(get("/api/v1/payment/flash-deals")).andExpect(status().isOk());
    }
  }
  ...
  class CreateOrder {
    ...
    void rejectsFreePlan() throws Exception {
      mockMvc
          .perform(post("/api/v1/payment/create-order").param("plan", "FREE"))
    ...
    void returns404WhenUserMissing() throws Exception {
      ...
      mockMvc
          .perform(post("/api/v1/payment/create-order").param("plan", "BASIC"))
      ...
      mockMvc
          .perform(
              post("/api/v1/payment/create-order")
                  .param("plan", "BASIC")
                  .param("discountCode", "FREE100"))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.data.isPremium").value(true))
          .andExpect(jsonPath("$.data.amount").value(0));

      verify(payOSService, never())
          .createPaymentLink(anyString(), any(), anyLong(), org.mockito.ArgumentMatchers.anyInt());
    ...
    void createsPendingTransactionWithCheckoutUrl() throws Exception {
      ...
      when(payOSService.createPaymentLink(
              eq(USER_ID), eq(SubscriptionPlan.BASIC), anyLong(), eq(199000)))
    ...
    void returnsErrorWhenPayOsFails() throws Exception {
      ...
      when(planService.getPlanByKey(SubscriptionPlan.BASIC))
          .thenReturn(PlanDefinition.builder().priceVnd(199000).build());
      when(payOSService.createPaymentLink(
              anyString(), any(), anyLong(), org.mockito.ArgumentMatchers.anyInt()))
  ...
  class CreateCourseOrder {
    ...
    void rejectsAlreadyPurchasedCourse() throws Exception {
      ...
      when(courseRepository.findById("course-1"))
          .thenReturn(Optional.of(Course.builder().id("course-1").build()));

      mockMvc
          .perform(post("/api/v1/payment/course-order").param("courseId", "course-1"))
          .andExpect(status().isBadRequest());
    }

    @Test
    @DisplayName(
        "400 VALIDATION_FAILED when a discountCode is supplied — not allowed for course orders")
    void rejectsDiscountCodeForCourseOrder() throws Exception {
      when(userRepository.findById(USER_ID)).thenReturn(Optional.of(baseUser()));
      when(courseRepository.findById("course-1"))
          .thenReturn(
              Optional.of(
                  Course.builder().id("course-1").priceVnd(199000).discountPercent(0).build()));
    ...
    void returns404WhenCourseMissing() throws Exception {
      ...
      mockMvc
          .perform(post("/api/v1/payment/course-order").param("courseId", "missing"))
          .andExpect(status().isNotFound());
    }
  ...
  class Webhook {
    ...
    @Test
    @DisplayName(
        "skips processing (idempotent) when transaction is already COMPLETED — DEFECT-002 guard")
    ...
    void grantsCoursePurchaseForCourseTransaction() throws Exception {
      ...
      User user = baseUser();
      when(userRepository.findById(USER_ID)).thenReturn(Optional.of(user));
      when(courseEnrollmentRepository.existsByUserIdAndCourseId(USER_ID, "course-1"))
          .thenReturn(false);
  ...
  class GetPaymentStatus {
    ...
    void rejectsAccessToAnotherUsersStatus() throws Exception {
      mockMvc
          .perform(get("/api/v1/payment/status/{userId}", "other-user-id"))
    ...
    void allowsAccessToOwnStatus() throws Exception {
      ...
      mockMvc
          .perform(get("/api/v1/payment/status/{userId}", USER_ID))
          .andExpect(status().isOk())
  ...
  class AdminCompleteTransaction {
    ...
    void returns404WhenTransactionMissing() throws Exception {
      ...
      mockMvc
          .perform(post("/api/v1/payment/admin/complete/{id}", "missing"))
          .andExpect(status().isNotFound());
    }
    ...
    void rejectsAlreadyCompletedTransaction() throws Exception {
      ...
      mockMvc
          .perform(post("/api/v1/payment/admin/complete/{id}", "tx-1"))
          .andExpect(status().isBadRequest());
    }
