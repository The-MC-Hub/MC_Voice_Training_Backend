...
/**
 * @WebMvcTest for VoiceController. Covers audio-file validation (content-type allowlist, 20MB size
 * cap, magic-byte sniffing), the IDOR guard in getHistory() (manual SecurityContextHolder authority
 * check, not @PreAuthorize), and the guest-cooldown gate on the unauthenticated analyze-guest
 * endpoint.
 */
...
class VoiceControllerTest {
  ...
  private void authenticateAs(String userId, String... authorities) {
    ...
    SecurityContextHolder.getContext()
        .setAuthentication(new UsernamePasswordAuthenticationToken(userId, null, auths));
  }
  ...
  class GetLessons {
    ...
    void delegatesToSearchWhenSearchPresent() throws Exception {
      ...
      mockMvc
          .perform(get("/api/v1/voice/lessons").param("search", "wedding"))
    ...
    void delegatesToCategoryWhenCategoryPresentNoSearch() throws Exception {
      ...
      mockMvc
          .perform(get("/api/v1/voice/lessons").param("category", "WEDDING"))
          .andExpect(status().isOk());
  ...
  class AnalyzePractice {
    ...
    void rejectsUnsupportedContentType() throws Exception {
      ...
      MockMultipartFile file =
          new MockMultipartFile(
              "audioFile", "malware.exe", "application/x-msdownload", VALID_WAV_BYTES);

      mockMvc
          .perform(
              multipart("/api/v1/voice/practice/analyze-voice")
                  .file(file)
                  .param("lessonId", "lesson-1"))
    ...
    void rejectsOversizedFile() throws Exception {
      ...
      MockMultipartFile file =
          new MockMultipartFile("audioFile", "big.wav", "audio/wav", oversized);
    ...
    @Test
    @DisplayName(
        "400 VALIDATION_FAILED when content-type claims audio/wav but magic bytes don't match")
    void rejectsSpoofedContentType() throws Exception {
      ...
      MockMultipartFile file =
          new MockMultipartFile(
              ...
              "fake.wav",
              "audio/wav",
              "not a real wav file content".getBytes(StandardCharsets.UTF_8));
    ...
    void acceptsValidWavAndDelegates() throws Exception {
      authenticateAs(USER_ID, "CLIENT");
      MockMultipartFile file =
          new MockMultipartFile("audioFile", "test.wav", "audio/wav", VALID_WAV_BYTES);
      when(voiceService.analyzePractice("lesson-1", USER_ID, file))
          .thenReturn(new PracticeSessionResponseDTO());
    ...
    void acceptsContentTypeWithCodecSuffix() throws Exception {
      ...
      MockMultipartFile file =
          new MockMultipartFile("audioFile", "test.webm", "audio/webm;codecs=opus", webmBytes);
      when(voiceService.analyzePractice(anyString(), anyString(), any()))
          .thenReturn(new PracticeSessionResponseDTO());
  ...
  class GetHistory {
    ...
    void rejectsAccessToAnotherUsersHistory() throws Exception {
      ...
      mockMvc
          .perform(get("/api/v1/voice/practice/history/{userId}", "other-user"))
    ...
    void allowsAccessToOwnHistory() throws Exception {
      ...
      mockMvc
          .perform(get("/api/v1/voice/practice/history/{userId}", USER_ID))
          .andExpect(status().isOk());
    }
  ...
  class AnalyzeGuestVoice {
    ...
    void rejectsWithinCooldown() throws Exception {
      when(guestUsageRepo.findById(anyString()))
          .thenReturn(
              Optional.of(
                  GuestVoiceUsage.builder()
                      .ipAddress("127.0.0.1")
      ...
      mockMvc
          .perform(multipart("/api/v1/voice/practice/analyze-guest").file(file))
          .andExpect(status().isBadRequest());
    ...
    void allowsFirstTimeGuestAndSavesUsage() throws Exception {
      ...
      when(systemSettingRepo.findById("GUEST_COOLDOWN_HOURS")).thenReturn(Optional.empty());
      when(voiceService.proxyAnalyzeVoice(any(), any()))
      ...
      when(guestUsageRepo.findById(anyString()))
          .thenReturn(
              Optional.of(
                  GuestVoiceUsage.builder()
                      .ipAddress("127.0.0.1")
                      .lastUsedAt(LocalDateTime.now().minusHours(10))
                      .build()));
      when(systemSettingRepo.findById("GUEST_COOLDOWN_HOURS")).thenReturn(Optional.empty());
      when(voiceService.proxyAnalyzeVoice(any(), any()))
          .thenReturn(java.util.Map.of("status", "success"));
  ...
  class GetAdaptiveStats {
    ...
    void returnsNullDataWhenNoStats() throws Exception {
      ...
      mockMvc
          .perform(get("/api/v1/voice/lessons/{id}/adaptive-stats", "lesson-1"))
          .andExpect(status().isOk())
  ...
  class GetGuestCooldownHours {
    ...
    void defaultsToThree() throws Exception {
      ...
      mockMvc
          .perform(get("/api/v1/voice/guest-cooldown-hours"))
          .andExpect(status().isOk())
