...
class UserHighlightControllerTest {
  ...
  void setUp() {
    SecurityContextHolder.getContext()
        .setAuthentication(new UsernamePasswordAuthenticationToken(OWNER_ID, null, List.of()));
  }
  ...
  @Nested
  @DisplayName(
      "GET /api/v1/highlights/reading-guides/{guideId} — DEFECT-001 fix: no userId in path")
  class GetHighlights {

    @Test
    @DisplayName("uses caller's userId from JWT, never a URL parameter")
    void usesCallerIdFromJwt() throws Exception {
      when(highlightService.getHighlights(OWNER_ID, "guide-1")).thenReturn(List.of());

      mockMvc
          .perform(get("/api/v1/highlights/reading-guides/{guideId}", "guide-1"))
  ...
  class CreateHighlight {
    ...
    void ignoresClientSuppliedUserId() throws Exception {
      ...
      when(highlightService.createHighlight(eq(OWNER_ID), any(UserHighlight.class)))
          .thenReturn(created);
  ...
  class UpdateHighlight {
    ...
    void returns404ForUnknownId() throws Exception {
      ...
      mockMvc
          .perform(
              put("/api/v1/highlights/{id}", "missing")
                  .contentType(MediaType.APPLICATION_JSON)
                  .content("{}"))
  ...
  class DeleteHighlight {
    ...
    void rejectsDeletingAnotherUsersHighlight() throws Exception {
      doThrow(new AppException(ErrorCode.ACCESS_DENIED, "Access denied"))
          ...
          .deleteHighlight(OWNER_ID, "h1");

      mockMvc.perform(delete("/api/v1/highlights/{id}", "h1")).andExpect(status().isForbidden());
    }
