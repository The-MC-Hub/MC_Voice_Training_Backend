...
/**
 * Regression guard for the 3-day purge floor (also covered at the service layer in
 * AuditLogServiceImplTest) — here we confirm the controller reports the enforced (floored) value
 * back to the caller, not the raw requested one.
 */
...
class AuditLogControllerTest {
  ...
  class Queries {
    ...
    void delegatesUserLogs() throws Exception {
      ...
      mockMvc
          .perform(get("/api/v1/audit-logs/user/{userId}", "target-user"))
  ...
  class PurgeLogs {
    ...
    void reportsFlooredValue() throws Exception {
      ...
      mockMvc
          .perform(delete("/api/v1/audit-logs/purge").param("days", "1"))
          .andExpect(status().isOk())
    ...
    void reportsRequestedValueWhenAboveFloor() throws Exception {
      ...
      mockMvc
          .perform(delete("/api/v1/audit-logs/purge").param("days", "90"))
          .andExpect(status().isOk())
    ...
    void logsAuditEntry() throws Exception {
      ...
      verify(auditLogService)
          .log(
              ...
              any(),
              any(),
              org.mockito.ArgumentMatchers.contains("\"enforcedDays\":3"),
              any());
    }
  }
}
