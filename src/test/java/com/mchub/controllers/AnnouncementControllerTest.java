...
class AnnouncementControllerTest {
      ...
      mockMvc
          .perform(
              post("/api/v1/admin/announcements")
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(
                      "{\"title\":\"Title\",\"content\":\"Body\",\"emailSubject\":\"Custom Subject\"}"))
          .andExpect(status().isOk())
  ...
  class Send {
    ...
    void passesNullRecipientIdsWhenBodyOmitted() throws Exception {
      when(announcementService.getById("ann-1"))
          .thenReturn(
      ...
      mockMvc
          .perform(post("/api/v1/admin/announcements/{id}/send", "ann-1"))
          .andExpect(status().isOk());
    ...
    void forwardsRecipientIdsWhenPresent() throws Exception {
      ...
      mockMvc
          .perform(
              post("/api/v1/admin/announcements/{id}/send", "ann-1")
                  .contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    @DisplayName(
        "rejects with 400 when announcement is already SENT, without calling approveAndSend")
  ...
  class EmailPreview {
    ...
    void returnsRawHtml() throws Exception {
      when(announcementService.renderEmailPreview("ann-1"))
          .thenReturn("<html><body>Preview</body></html>");

      mockMvc
          .perform(get("/api/v1/admin/announcements/{id}/email-preview", "ann-1"))
          .andExpect(status().isOk())
          .andExpect(
              org.springframework.test.web.servlet.result.MockMvcResultMatchers.content()
                  .string("<html><body>Preview</body></html>"));
    }
  }
  ...
  class TriggerNewLesson {
    ...
    void usesDefaultLessonTitle() throws Exception {
      ...
      mockMvc
          .perform(
              post("/api/v1/admin/announcements/trigger/new-lesson")
                  .contentType(MediaType.APPLICATION_JSON)
                  .content("{}"))
          .andExpect(status().isOk());
  ...
  class TriggerDiscount {
    ...
    void alwaysTargetsFreePlan() throws Exception {
      ...
      mockMvc
          .perform(
              post("/api/v1/admin/announcements/trigger/discount")
                  .contentType(MediaType.APPLICATION_JSON)
      ...
      verify(announcementService)
          .createFromTrigger(
              eq(Announcement.AnnouncementType.DISCOUNT),
              any(),
              any(),
              any(),
              eq("SAVE20"),
              eq("DiscountCode"),
              eq(java.util.List.of("FREE")));
    }
  }
}
