...
class VoucherControllerTest {
  ...
  class GetAvailableVouchers {
    ...
    void filtersOutExpired() throws Exception {
      UserVoucher valid =
          UserVoucher.builder().userId(USER_ID).expiresAt(LocalDateTime.now().plusDays(1)).build();
      when(voucherService.getAvailableVouchers(USER_ID)).thenReturn(List.of(valid));

      mockMvc
          .perform(get("/api/v1/vouchers/my/available"))
    ...
    void includesVouchersWithNoExpiry() throws Exception {
      ...
      when(voucherService.getAvailableVouchers(USER_ID)).thenReturn(List.of(noExpiry));
