...
class EmailCampaignControllerTest {
  ...
  class TemplateEndpoints {
    ...
    void createsTemplateWithDesignData() throws Exception {
      when(emailCampaignService.createTemplate(
              eq("Welcome"), eq("Hi"), isNull(), any(EmailTemplate.DesignData.class)))
      ...
      mockMvc
          .perform(
              post("/api/v1/admin/email/templates")
                  ...
                  .content(
                      "{\"name\":\"Welcome\",\"subject\":\"Hi\",\"designData\":{\"title\":\"Hello\"}}"))
    ...
    void usesEmptyDesignDataWhenNull() throws Exception {
      when(emailCampaignService.createTemplate(
              any(), any(), any(), any(EmailTemplate.DesignData.class)))
          .thenReturn(EmailTemplateDTO.builder().id("tpl-1").build());
  ...
  class DeleteTemplate {
    ...
    void deletesTemplate() throws Exception {
      mockMvc
          .perform(delete("/api/v1/admin/email/templates/{id}", "tpl-1"))
          .andExpect(status().isOk());
  ...
  class SendCampaign {
    ...
    void defaultsTargetTypeToAll() throws Exception {
      when(emailCampaignService.sendCampaign(
              eq("tpl-1"), eq("Subject"), eq("ALL"), any(), any(), any()))
    ...
    void forwardsExplicitTargeting() throws Exception {
      when(emailCampaignService.sendCampaign(
              eq("tpl-1"), eq("Subject"), eq("PLAN"), any(), any(), any()))
      ...
      mockMvc
          .perform(
              post("/api/v1/admin/email/campaigns/send")
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(
                      "{\"templateId\":\"tpl-1\",\"subject\":\"Subject\",\"targetType\":\"PLAN\",\"targetPlans\":[\"BASIC\"]}"))
          .andExpect(status().isOk());
    }
  }
  ...
  class CountRecipients {
    ...
    void countsRecipients() throws Exception {
      ...
      mockMvc
          .perform(
              post("/api/v1/admin/email/campaigns/count-recipients")
                  .contentType(MediaType.APPLICATION_JSON)
                  .content("{}"))
          .andExpect(status().isOk());
    }
  ...
  class GetCampaignLogs {
    ...
    void returnsLogs() throws Exception {
      ...
      mockMvc
          .perform(get("/api/v1/admin/email/campaigns/{id}/logs", "camp-1"))
          .andExpect(status().isOk());
    }
  }
}
