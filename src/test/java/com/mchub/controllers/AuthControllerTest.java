...
/**
 * @WebMvcTest for AuthController — loads only the web layer (controller + GlobalExceptionHandler),
 * all service dependencies mocked. Security filters disabled (addFilters=false) since
 * /api/v1/auth/** is permitAll in SecurityConfig anyway and JwtAuthenticationFilter needs a live
 * JwtService bean we don't want to wire here; endpoints requiring auth (/me,
 * /referral-code/generate, /settings) are exercised via SecurityUtils directly, which reads
 * SecurityContextHolder — populated per-test as needed.
 */
...
class AuthControllerTest {
  ...
  class Register {
    ...
    @Test
    @DisplayName(
        "registration still succeeds (still 201) even if sendOtp throws — exception is swallowed")
          ...
          .perform(
              post("/api/v1/auth/reset-password")
                  ...
                  .content(
                      "{\"email\":\""
                          + EMAIL
                          + "\",\"code\":\"111111\",\"newPassword\":\"newpass123\"}"))
