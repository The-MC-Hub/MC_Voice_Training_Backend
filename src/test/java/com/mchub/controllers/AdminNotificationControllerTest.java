...
class AdminNotificationControllerTest {
  ...
  class GetAllNotifications {
    ...
    void returnsAllNotifications() throws Exception {
      when(notificationRepository.findAll())
          .thenReturn(List.of(Notification.builder().id("n1").userId("u1").build()));
  ...
  class SendNotification {
      ...
      mockMvc
          .perform(
              post("/api/v1/admin/notifications/send")
                  ...
                  .content(
                      "{\"targetType\":\"USER\",\"userId\":\"u1\",\"type\":\"ANNOUNCEMENT\",\"title\":\"Hi\",\"body\":\"Body\"}"))
    ...
    void sendsToRole() throws Exception {
      when(userRepository.findByRole(UserRole.MC))
          .thenReturn(List.of(User.builder().id("mc1").build(), User.builder().id("mc2").build()));
      ...
      mockMvc
          .perform(
              post("/api/v1/admin/notifications/send")
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(
                      "{\"targetType\":\"ROLE\",\"role\":\"MC\",\"type\":\"ANNOUNCEMENT\",\"title\":\"Hi\",\"body\":\"Body\"}"))
          .andExpect(status().isOk());
    ...
    void sendsToAllExcludingAdmins() throws Exception {
      when(userRepository.findByRoleNot(UserRole.ADMIN))
          .thenReturn(
              List.of(
      ...
      mockMvc
          .perform(
              post("/api/v1/admin/notifications/send")
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(
                      "{\"targetType\":\"ALL\",\"type\":\"ANNOUNCEMENT\",\"title\":\"Hi\",\"body\":\"Body\"}"))
          .andExpect(status().isOk());
      ...
      mockMvc
          .perform(
              post("/api/v1/admin/notifications/send")
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(
                      "{\"targetType\":\"USER\",\"type\":\"ANNOUNCEMENT\",\"title\":\"Hi\",\"body\":\"Body\"}"))
          .andExpect(status().isBadRequest());

      verify(notificationService, never())
          .notify(anyString(), any(), anyString(), anyString(), any(), eq(false));
    }
      ...
      mockMvc
          .perform(
              post("/api/v1/admin/notifications/send")
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(
                      "{\"targetType\":\"ALL\",\"type\":\"ANNOUNCEMENT\",\"title\":\"\",\"body\":\"Body\"}"))
          .andExpect(status().isBadRequest());
    }
  }
  ...
  class DeleteNotification {
    ...
    void returns404ForMissing() throws Exception {
      ...
      mockMvc
          .perform(delete("/api/v1/admin/notifications/{id}", "missing"))
