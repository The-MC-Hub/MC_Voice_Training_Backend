 ...
 * verifyCertificate/deleteCertificate always throw UnsupportedOperationException, which
 * GlobalExceptionHandler maps to HTTP 410 Gone. Only the GET (getCertificatesByMCProfile,
 * repurposed as a userId lookup) still works.
 */
...
class CertificateControllerTest {
  ...
  class GetCertificates {
    ...
    void returnsMappedCertificates() throws Exception {
      when(certificateService.getCertificatesByMCProfile("mc-1"))
          .thenReturn(List.of(new Certificate()));

      mockMvc
          .perform(get("/api/v1/certificates/mc/{mcProfileId}", "mc-1"))
  ...
  class Verify {
    ...
    void returns410ForDeprecatedFeature() throws Exception {
      when(certificateService.verifyCertificate(
              org.mockito.ArgumentMatchers.anyString(), org.mockito.ArgumentMatchers.anyString()))
          .thenThrow(new UnsupportedOperationException("deprecated"));

      mockMvc
          .perform(put("/api/v1/certificates/{id}/verify", "cert-1"))
          .andExpect(status().isGone());
    }
  }
  ...
  class DeleteCertificate {
    ...
    void returns404WhenNoMcProfile() throws Exception {
      ...
      mockMvc
          .perform(delete("/api/v1/certificates/{id}", "cert-1"))
          .andExpect(status().isNotFound());
    }
  }
}
