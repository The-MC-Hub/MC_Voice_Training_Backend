  ...
  class UploadMedia {
    ...
    void returnsUrlOnSuccess() throws Exception {
      when(mediaService.uploadFile(any(), anyString()))
          .thenReturn("https://cloudinary.com/img.jpg");
      MockMultipartFile file =
          new MockMultipartFile("file", "test.jpg", "image/jpeg", "content".getBytes());

      mockMvc
          .perform(multipart("/api/v1/media/upload").file(file))
    ...
    void usesDefaultFolder() throws Exception {
      ...
      org.mockito.Mockito.verify(mediaService)
          .uploadFile(any(), org.mockito.ArgumentMatchers.eq("mchub_chat"));
    }

    @Test
    @DisplayName(
        "500 with fail envelope when upload throws — caught inline, not via GlobalExceptionHandler")
    void returns500WithFailEnvelopeOnUploadFailure() throws Exception {
      when(mediaService.uploadFile(any(), anyString()))
          .thenThrow(new java.io.IOException("Cloudinary down"));
      MockMultipartFile file =
          new MockMultipartFile("file", "test.jpg", "image/jpeg", "content".getBytes());
