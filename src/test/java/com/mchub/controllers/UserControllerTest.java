...
/**
 * Regression guard for the audit finding (Remaining_Modules_Audit_Report.md 3.7, naming issue):
 * POST /me/streak/freeze does NOT actually consume a freeze — it only re-displays the current
 * streak snapshot. The freeze is consumed elsewhere, inside
 * GamificationServiceImpl.processLoginStreak() when a 2-day login gap is detected. Tests here
 * confirm the endpoint's TRUE (read-only) behavior, not the name's implication.
 */
...
class UserControllerTest {
  ...
  class GetLoginStreak {
    ...
    void resolvesNoneFrameAtZero() throws Exception {
      when(gamificationService.getOrCreateUserStats(USER_ID))
          .thenReturn(baseStats().loginStreak(0).build());

      mockMvc
          .perform(get("/api/v1/users/me/streak"))
    ...
    void resolvesImmortalAtHundredPlus() throws Exception {
      when(gamificationService.getOrCreateUserStats(USER_ID))
          .thenReturn(baseStats().loginStreak(150).build());
    ...
    void resolvesFlameAtSevenDayBoundary() throws Exception {
      when(gamificationService.getOrCreateUserStats(USER_ID))
          .thenReturn(baseStats().loginStreak(7).build());
  ...
  class UseFreeze {
    ...
    @Test
    @DisplayName(
        "400 with fail envelope when no freezes available — does not call processLoginStreak")
    void returns400WhenNoFreezesAvailable() throws Exception {
      when(gamificationService.getOrCreateUserStats(USER_ID))
          .thenReturn(baseStats().freezesAvailable(0).build());

      mockMvc
          .perform(post("/api/v1/users/me/streak/freeze"))
          .andExpect(status().isBadRequest())
          .andExpect(jsonPath("$.status").value("fail"));

      org.mockito.Mockito.verify(gamificationService, org.mockito.Mockito.never())
          .processLoginStreak(USER_ID);
    }

    @Test
    @DisplayName(
        "200 OK just re-returns the current streak snapshot — does NOT decrement freezesAvailable itself")
    void returnsCurrentSnapshotWithoutConsumingFreeze() throws Exception {
      ...
      mockMvc
          ...
          .andExpect(status().isOk())
          .andExpect(
              jsonPath("$.data.freezesAvailable")
                  .value(1)); // unchanged — this endpoint never consumes it
