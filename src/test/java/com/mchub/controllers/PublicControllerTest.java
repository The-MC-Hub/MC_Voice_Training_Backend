...
class PublicControllerTest {
  ...
  class GetMcProfile {
    ...
    void returnsProfileWrapped() throws Exception {
      ...
      mockMvc
          .perform(get("/api/v1/public/mcs/{id}", "mc-1"))
  ...
  class DiscoverMcs {
    ...
    void wrapsListWithCount() throws Exception {
      when(publicService.discoverMCs(any()))
          .thenReturn(List.of(new MCProfileResponseDTO(), new MCProfileResponseDTO()));

      mockMvc
          .perform(get("/api/v1/public/mcs"))
          .andExpect(status().isOk())
