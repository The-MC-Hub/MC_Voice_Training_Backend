...
/**
 * @WebMvcTest for AdminController. Class-level @PreAuthorize("hasAuthority('ADMIN')") is not
 * enforced in this slice (method security requires the full app context) — role-boundary testing is
 * covered by SecurityConfig manual review and UC-09 system testing. Here we exercise request
 * validation, delegation to AdminService, and audit logging side effects. migrateDb is deliberately
 * NOT exercised end-to-end per DEFECT-003 (hardcoded production DB name) — only the controller
 * wiring (that it calls migrationService.migrateFromMcHub()) is verified via a mock, never a real
 * call.
 */
...
class AdminControllerTest {
  ...
  void setUp() {
    SecurityContextHolder.getContext()
        .setAuthentication(new UsernamePasswordAuthenticationToken(ADMIN_ID, null, List.of()));
  }
  ...
  class ReadOnlyDelegation {
    ...
    void usersMcsDelegates() throws Exception {
      ...
      mockMvc.perform(get("/api/v1/admin/users/mcs")).andExpect(status().isOk());
    }
  }
  ...
  class UpdateUserStatus {
    ...
    void rejectsMissingFields() throws Exception {
      mockMvc
          .perform(
              put("/api/v1/admin/users/{id}/status", TARGET_ID)
  ...
  class UpdateUserPlan {
    ...
    void rejectsBlankPlan() throws Exception {
      mockMvc
          .perform(
              put("/api/v1/admin/users/{id}/plan", TARGET_ID)
                  .contentType(MediaType.APPLICATION_JSON)
                  .content("{\"plan\":\"\"}"))
          .andExpect(status().isBadRequest());
    }
      ...
      mockMvc
          .perform(
              post("/api/v1/admin/users")
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(
                      "{\"name\":\"Test\",\"email\":\"t@test.local\",\"password\":\"pass1234\"}"))
          .andExpect(status().isOk())
      ...
      mockMvc
          .perform(
              post("/api/v1/admin/users")
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(
                      "{\"name\":\"Test\",\"email\":\"taken@test.local\",\"password\":\"pass1234\"}"))
  ...
  class ChangePassword {
    ...
    void rejectsBlankPassword() throws Exception {
      mockMvc
          .perform(
              post("/api/v1/admin/users/{id}/change-password", TARGET_ID)
                  .contentType(MediaType.APPLICATION_JSON)
                  .content("{\"newPassword\":\"\"}"))
          .andExpect(status().isBadRequest());
  ...
  class DeleteUser {
    ...
    void deletesAndLogsAudit() throws Exception {
      mockMvc.perform(delete("/api/v1/admin/users/{id}", TARGET_ID)).andExpect(status().isOk());
  ...
  class NotifyEmail {
    ...
    void rejectsBlankSubjectOrContent() throws Exception {
      mockMvc
          .perform(
              post("/api/v1/admin/users/{id}/notify-email", TARGET_ID)
                  .contentType(MediaType.APPLICATION_JSON)
  ...
  class MigrateDb {
    ...
    void callsMigrationServiceOnce() throws Exception {
      mockMvc.perform(post("/api/v1/admin/migrate-db")).andExpect(status().isOk());

      verify(migrationService).migrateFromMcHub();
      verify(auditLogService)
          .log(org.mockito.ArgumentMatchers.eq(ADMIN_ID), any(), anyString(), any(), any(), any());
    }
  }
  ...
  class GuestCooldownSettings {
    ...
    void defaultsToThreeHoursWhenUnset() throws Exception {
      ...
      mockMvc
          .perform(get("/api/v1/admin/settings/guest-cooldown"))
          .andExpect(status().isOk())
    ...
    void rejectsHoursBelowMin() throws Exception {
      mockMvc
          .perform(put("/api/v1/admin/settings/guest-cooldown").param("hours", "0"))
          .andExpect(status().isBadRequest());
    }
    ...
    void rejectsHoursAboveMax() throws Exception {
      mockMvc
          .perform(put("/api/v1/admin/settings/guest-cooldown").param("hours", "200"))
          .andExpect(status().isBadRequest());
    }
    ...
    void acceptsUpperBoundary() throws Exception {
      ...
      mockMvc
          .perform(put("/api/v1/admin/settings/guest-cooldown").param("hours", "168"))
          .andExpect(status().isOk());

      verify(systemSettingRepo)
          .save(org.mockito.ArgumentMatchers.argThat(s -> "168".equals(s.getValue())));
    }
  }
}
