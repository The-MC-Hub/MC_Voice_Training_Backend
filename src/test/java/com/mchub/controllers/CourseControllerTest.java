...
class CourseControllerTest {
  ...
  class ListCourses {
    ...
    void delegatesToTypeFilterWhenPresent() throws Exception {
      ...
      mockMvc
          .perform(get("/api/v1/courses").param("type", "WEDDING_MC"))
  ...
  class CompleteLesson {
    ...
    void delegatesWithPathVariables() throws Exception {
      ...
      mockMvc
          .perform(post("/api/v1/courses/{id}/lessons/{lessonId}/complete", "course-1", "lesson-1"))
          .andExpect(status().isOk());
    }
  }
  ...
  class SubmitQuiz {
    ...
    void rejectsEmptyAnswers() throws Exception {
      ...
      mockMvc
          .perform(
              post("/api/v1/courses/{id}/quiz/submit", "course-1")
    ...
    void submitsValidAnswers() throws Exception {
      authenticate();
      when(courseService.submitQuiz(eq("course-1"), eq(USER_ID), any()))
          .thenReturn(new QuizResultDTO());
