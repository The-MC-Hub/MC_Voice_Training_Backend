...
/**
 * @WebMvcTest for PeerReviewController. Covers the ownership check on requestReview() (learner can
 * only request review for their own session), the duplicate-request guard, and the submit()/toDTO()
 * enrichment path.
 */
...
class PeerReviewControllerTest {
  ...
  class RequestReview {
    ...
    void notFoundWhenSessionMissing() throws Exception {
      ...
      mockMvc
          .perform(post("/api/v1/peer-review/request/{id}", SESSION_ID))
    ...
    void rejectsWhenNotOwner() throws Exception {
      PracticeSession session =
          PracticeSession.builder().id(SESSION_ID).userId(OTHER_USER_ID).build();
    ...
    void createsReviewWhenEligible() throws Exception {
      PracticeSession session =
          PracticeSession.builder().id(SESSION_ID).userId(USER_ID).lessonId("lesson-1").build();
      when(practiceSessionRepository.findById(SESSION_ID)).thenReturn(Optional.of(session));
  ...
  class SubmitReview {
    ...
    void notFoundWhenReviewMissing() throws Exception {
      ...
      mockMvc
          .perform(
              post("/api/v1/peer-review/{id}/submit", "review-1")
  ...
  class ForSession {
    ...
    void returnsNullWhenNoReview() throws Exception {
      when(practiceReviewRepository.findByPracticeSessionId(SESSION_ID))
          .thenReturn(Optional.empty());

      mockMvc
          .perform(get("/api/v1/peer-review/session/{id}", SESSION_ID))
          .andExpect(status().isOk())
  ...
  class Pending {
    ...
    void returnsPendingReviews() throws Exception {
      ...
      when(userRepository.findById(USER_ID))
          .thenReturn(Optional.of(User.builder().id(USER_ID).name("Learner A").build()));

      mockMvc
          .perform(get("/api/v1/peer-review/pending"))
          .andExpect(status().isOk())
