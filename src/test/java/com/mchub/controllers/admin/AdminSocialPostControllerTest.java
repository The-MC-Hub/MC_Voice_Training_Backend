...
class AdminSocialPostControllerTest {
  ...
  class Create {
    ...
    void createsWithValidPayload() throws Exception {
      when(socialPostService.createPost(any()))
          .thenReturn(SocialPostDTO.builder().id("new-id").build());
  ...
  class Delete {
    ...
    void deletesPost() throws Exception {
      mockMvc
          .perform(delete("/api/v1/admin/social-posts/{id}", "post-1"))
          .andExpect(status().isOk());
  ...
  class Toggle {
    ...
    void togglesActive() throws Exception {
      when(socialPostService.toggleActive("post-1"))
          .thenReturn(SocialPostDTO.builder().id("post-1").active(false).build());

      mockMvc
          .perform(patch("/api/v1/admin/social-posts/{id}/toggle", "post-1"))
          .andExpect(status().isOk());
    }
  }
}
