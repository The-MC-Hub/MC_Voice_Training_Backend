...
class AdminPlanControllerTest {
  ...
  class PlanCrud {
    ...
    void updatesPlan() throws Exception {
      ...
      mockMvc
          .perform(
              put("/api/v1/admin/plans/{id}", "plan-1")
  ...
  class SeedDailyPlan {
    ...
    void returnsExistingWhenAlreadySeeded() throws Exception {
      PlanDefinition existing =
          PlanDefinition.builder().id("daily-1").plan(SubscriptionPlan.DAILY).build();
      when(planService.getPlanByKey(SubscriptionPlan.DAILY)).thenReturn(existing);

      mockMvc
          .perform(post("/api/v1/admin/plans/seed-daily"))
          .andExpect(status().isOk())
  ...
  class DiscountCrud {
    ...
    void forcesIdNullAndUsedCountZero() throws Exception {
      ...
      mockMvc
          .perform(
              post("/api/v1/admin/plans/discounts")
                  .contentType(MediaType.APPLICATION_JSON)
    ...
    void updatesDiscount() throws Exception {
      ...
      mockMvc
          .perform(
              put("/api/v1/admin/plans/discounts/{id}", "disc-1")
                  .contentType(MediaType.APPLICATION_JSON)
                  .content("{\"code\":\"NEW10\"}"))
          .andExpect(status().isOk());
    }
  ...
  class DeleteDiscount {
    ...
    void deletesDiscount() throws Exception {
      mockMvc
          .perform(delete("/api/v1/admin/plans/discounts/{id}", "disc-1"))
          .andExpect(status().isOk());
