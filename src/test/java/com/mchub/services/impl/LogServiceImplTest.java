...
/**
 * Unit tests for LogServiceImpl. Covers the query-dispatch logic in getLogs() (4-way branch on
 * level/source presence) and the external AI-service log ingest path. SSE streaming/broadcast and
 * the @PostConstruct Java-log listener wiring are integration concerns, not covered here.
 */
...
class LogServiceImplTest {
  ...
  class GetLogs {
    ...
    @Test
    @DisplayName(
        "both level and source present: uses findByLevelAndSourceOrderByTimestampDesc, uppercased")
    void bothLevelAndSource() {
      service.getLogs("error", "java", 200);

      verify(logRepository)
          .findByLevelAndSourceOrderByTimestampDesc(
    ...
    void onlyLevel() {
      ...
      verify(logRepository)
          .findByLevelOrderByTimestampDesc(
    ...
    void onlySource() {
      ...
      verify(logRepository)
          .findBySourceOrderByTimestampDesc(
    ...
    void neitherPresent() {
      ...
      verify(logRepository)
          .findByOrderByTimestampDesc(any(org.springframework.data.domain.Pageable.class));
    }
    ...
    void limitHonoredAndCapped() {
      ...
      verify(logRepository, org.mockito.Mockito.times(2))
          .findByOrderByTimestampDesc(captor.capture());
  ...
  class IngestExternal {
    ...
    void setsTimestampWhenMissing() {
      ...
      assertThat(entry.getTimestamp())
          .isCloseTo(
              Instant.now(),
              org.assertj.core.api.Assertions.within(3, java.time.temporal.ChronoUnit.SECONDS));
    }
