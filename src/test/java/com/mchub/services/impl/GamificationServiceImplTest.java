...
/**
 * Unit tests for GamificationServiceImpl. Mocks all repositories — no real DB. Covers XP/tier
 * calculation, practice-streak and login-streak logic (including the freeze mechanic), badge
 * idempotency, and the fire-and-forget competition update path.
 */
...
class GamificationServiceImplTest {
  ...
  void setUp() {
    ...
    org.mockito.Mockito.lenient()
        .when(competitionRepository.findByActive(true))
        .thenReturn(List.of());
  }
  ...
  private UserStats.UserStatsBuilder baseStats() {
    return UserStats.builder()
        ...
        .currentTier("BRONZE")
        .weeklyXP(0.0)
        .earnedBadges(new java.util.ArrayList<>());
  }
  ...
  class PracticeStreak {
    ...
    void longestStreakTracksMax() {
      Instant threeDaysAgo = Instant.now().minus(3, ChronoUnit.DAYS);
      UserStats stats =
          baseStats().currentStreak(10).longestStreak(10).lastPracticeTime(threeDaysAgo).build();
      when(userStatsRepository.findByUserId(USER_ID)).thenReturn(Optional.of(stats));
      when(userStatsRepository.save(any(UserStats.class))).thenAnswer(inv -> inv.getArgument(0));

      service.processPracticeSession(
          USER_ID, "lesson-1", 50.0, 50.0, 50.0, 0.0); // resets currentStreak to 1
  ...
  class BadgeIdempotency {
    ...
    void awardsSessions10Once() {
      ...
      service.processPracticeSession(
          USER_ID, "lesson-1", 50.0, 50.0, 50.0, 0.0); // totalSessions -> 10
      assertThat(stats.getEarnedBadges()).containsExactlyInAnyOrder("SESSIONS_10");

      service.processPracticeSession(
          USER_ID, "lesson-1", 50.0, 50.0, 50.0, 0.0); // totalSessions -> 11
      assertThat(stats.getEarnedBadges())
          .containsExactlyInAnyOrder("SESSIONS_10"); // not duplicated
    }
    ...
    void doesNotAwardBelowThreshold() {
      ...
      service.processPracticeSession(
          USER_ID, "lesson-1", 50.0, 50.0, 50.0, 0.0); // streak becomes 1
  ...
  class LoginStreak {
    ...
    void sameDayLoginIsNoOp() {
      UserStats stats =
          baseStats().loginStreak(5).lastLoginDate(LocalDate.now(ZoneId.systemDefault())).build();
      when(userStatsRepository.findByUserId(USER_ID)).thenReturn(Optional.of(stats));
    ...
    void oneDayGapIncrementsStreak() {
      UserStats stats =
          baseStats()
              ...
              .freezesAvailable(1)
              .lastFreezeGranted(LocalDate.now(ZoneId.systemDefault()))
              .build();
      when(userStatsRepository.findByUserId(USER_ID)).thenReturn(Optional.of(stats));
      when(userStatsRepository.save(any(UserStats.class))).thenAnswer(inv -> inv.getArgument(0));
  ...
  class Competitions {
    ...
    void createsNewRecordOnFirstAttempt() {
      ...
      Competition comp =
          Competition.builder().id("comp-1").active(true).challengeScriptId("lesson-1").build();
      when(competitionRepository.findByActive(true)).thenReturn(List.of(comp));
      when(userRepository.findById(USER_ID))
          .thenReturn(Optional.of(User.builder().id(USER_ID).name("QA Tester").build()));
      when(competitionRecordRepository.findByCompetitionIdAndUserId("comp-1", USER_ID))
          .thenReturn(Optional.empty());
      when(competitionRecordRepository.save(any(CompetitionRecord.class)))
    ...
    @Test
    @DisplayName(
        "does not create a record for a competition whose challengeScriptId does not match")
    void skipsNonMatchingCompetition() {
      ...
      Competition comp =
          Competition.builder().id("comp-1").active(true).challengeScriptId("other-lesson").build();
      when(competitionRepository.findByActive(true)).thenReturn(List.of(comp));
  ...
  class ProcessCourseCompletion {
    ...
    void awardsGraduateBadgeOnce() {
      UserStats stats =
          baseStats().earnedBadges(new java.util.ArrayList<>(List.of("COURSE_GRADUATE"))).build();
      when(userStatsRepository.findByUserId(USER_ID)).thenReturn(Optional.of(stats));
      when(userStatsRepository.save(any(UserStats.class))).thenAnswer(inv -> inv.getArgument(0));
      when(discountCodeRepository.findByCodeIgnoreCase(any())).thenReturn(Optional.empty());
      when(userVoucherRepository.existsByUserIdAndCode(any(), any())).thenReturn(false);
