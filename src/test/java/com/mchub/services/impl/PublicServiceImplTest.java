 ...
 * (Remaining_Modules_Audit_Report.md 2.3): getFeaturedMCTrainingStats() used to call
 * practiceSessionRepository.findByUserId() inside a per-profile loop — now batch-fetched once via
 * findByUserIdIn() before the loop.
 */
...
class PublicServiceImplTest {
  ...
  class GetFeaturedMCTrainingStats {
    ...
    @Test
    @DisplayName(
        "batch-fetches sessions for ALL MC profiles in a single call — never per-profile inside the loop")
    void batchFetchesSessionsOnce() {
      ...
      when(userRepository.findAllById(List.of("user-1", "user-2")))
          .thenReturn(
              List.of(
                  User.builder().id("user-1").name("MC One").build(),
                  User.builder().id("user-2").name("MC Two").build()));
      when(practiceSessionRepository.findByUserIdIn(List.of("user-1", "user-2")))
          .thenReturn(List.of());
    ...
    void computesAveragesPerProfile() {
      ...
      when(userRepository.findAllById(List.of("user-1")))
          .thenReturn(
              List.of(User.builder().id("user-1").name("MC One").avatar("avatar.png").build()));
  ...
  class EnumOptions {
    ...
    void formatsLabelAsTitleCase() {
      ...
      var adminOption =
          result.stream().filter(o -> o.getValue().equals("ADMIN")).findFirst().orElseThrow();
