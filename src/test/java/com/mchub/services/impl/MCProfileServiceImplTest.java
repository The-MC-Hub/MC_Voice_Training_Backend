...
/**
 * Unit tests for MCProfileServiceImpl. Verifies the DEFECT fix from the audit report (section 2.4):
 * getDashboardStats() used to call practiceSessionRepository.findByUserId() twice (once per
 * CompletableFuture) for the same data — now a single fetch is reused for both averages.
 */
...
class MCProfileServiceImplTest {
  ...
  void setUp() {
    service =
        new MCProfileServiceImpl(
            mcProfileRepository, practiceSessionRepository, recommendationService);
  }
  ...
  class GetDashboardStats {
      ...
      List<PracticeSession> sessions =
          List.of(
              PracticeSession.builder()
                  ...
                  .userId(USER_ID)
                  .accuracyScore(80.0)
                  .speakingRateWpm(140.0)
                  .build(),
              PracticeSession.builder()
                  .id("s2")
                  .userId(USER_ID)
                  .accuracyScore(90.0)
                  .speakingRateWpm(160.0)
                  .build());
    ...
    @Test
    @DisplayName(
        "only calls findByUserId ONCE — regression guard for the duplicate-fetch defect fixed in audit")
    void fetchesSessionsOnlyOnce() {
      ...
      org.mockito.Mockito.verify(practiceSessionRepository, org.mockito.Mockito.times(1))
  ...
  class UpdateProfile {
    ...
    @Test
    @DisplayName(
        "experience is preserved when update payload's is left at its zero-default (not a real 'no value')")
    void partialUpdatePreservesExperience() {
      MCProfile existing =
          MCProfile.builder().user(USER_ID).experience(5).biography("Old bio").build();
