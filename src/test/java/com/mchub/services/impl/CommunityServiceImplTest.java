 ...
 * (streak/precision/sessions/default/weekly), rank calculation (offset by page + 1-indexed),
 * user-rank lookup, and active arena leaderboard sorting by accuracy+rhythm.
 */
...
class CommunityServiceImplTest {
  ...
  void setUp() {
    service =
        new CommunityServiceImpl(
            userRepository,
            userStatsRepository,
            competitionRepository,
            competitionRecordRepository,
            voiceLessonRepository);
  }

  private UserStats stats(
      String userId, double hours, int sessions, double xp, double weeklyXp, int streak) {
  ...
  class GetCommunityStats {
    ...
    void sumsTotalPracticeHours() {
      ...
      when(userStatsRepository.findAll())
          .thenReturn(List.of(stats("u1", 5.0, 10, 0, 0, 0), stats("u2", 3.0, 8, 0, 0, 0)));
    ...
    void usesFirstLessonTitle() {
      when(userRepository.count()).thenReturn(0L);
      when(userStatsRepository.findAll()).thenReturn(List.of());
      when(voiceLessonRepository.findAll())
          .thenReturn(List.of(VoiceLesson.builder().id("l1").title("Custom Lesson Title").build()));
      when(competitionRepository.findByActive(true)).thenReturn(List.of());
  ...
  class GetLeaderboardDispatch {
    ...
    void unrecognizedTypeFallsBackToPracticeHours() {
      ...
      org.mockito.Mockito.verify(userStatsRepository)
          .findAllByOrderByTotalPracticeHoursDesc(pageable);
    }
  ...
  class GetActiveArenas {
    ...
    void sortsLeaderboardByAccuracyPlusRhythm() {
      ...
      when(voiceLessonRepository.findById("lesson-1"))
          .thenReturn(
              Optional.of(
                  VoiceLesson.builder().id("lesson-1").title("Title").content("Content").build()));

      CompetitionRecord low =
      ...
      CompetitionRecord high =
          CompetitionRecord.builder()
              ...
              .bestRhythm(90.0)
              .build();
      when(competitionRecordRepository.findByCompetitionId("comp-1"))
    ...
    void userRecordNullWhenUserIdNull() {
      Competition comp =
          Competition.builder().id("comp-1").type(CompetitionType.WEEKLY).active(true).build();
      when(competitionRepository.findByActive(true)).thenReturn(List.of(comp));
