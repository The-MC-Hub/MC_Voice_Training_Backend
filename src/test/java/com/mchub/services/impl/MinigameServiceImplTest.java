...
/**
 * Unit tests for MinigameServiceImpl (Speed Reader minigame). Covers prompt generation (difficulty
 * normalization, round-count clamping, timer budget), XP/score calculation on submit, personal-best
 * detection, and leaderboard DTO mapping with the MongoTemplate aggregation mocked directly.
 */
...
class MinigameServiceImplTest {
  ...
  void setUp() {
    service =
        new MinigameServiceImpl(
            minigameResultRepository, userRepository, gamificationService, mongoTemplate);
  }
  ...
  class GetSpeedReaderPrompts {
    ...
    @Test
    @DisplayName(
        "clamps roundCount to the pool size when more rounds requested than available lines")
    ...
    @Test
    @DisplayName(
        "HARD difficulty gives a stricter (larger) per-word time budget than EASY for equal word count")
  ...
  class SubmitSpeedReaderRun {
    ...
    void computesScoreCorrectly() {
      when(minigameResultRepository.findTopByUserIdAndGameTypeOrderByScoreDesc(
              USER_ID, "SPEED_READER"))
          .thenReturn(null);
    ...
    void newBestWhenScoreTiesOrBeats() {
      ...
      MinigameResultDTO result =
          service.submitSpeedReaderRun(USER_ID, request("EASY", 3, 0)); // score=300
    ...
    @Test
    @DisplayName(
        "isNewPersonalBest=false and personalBestScore stays at the higher prior value when new score is lower")
    void notNewBestWhenScoreLower() {
      ...
      MinigameResultDTO result =
          service.submitSpeedReaderRun(USER_ID, request("EASY", 1, 0)); // score=100
    ...
    void savesResultWithCorrectGameType() {
      ...
      org.mockito.Mockito.verify(minigameResultRepository)
          .save(
              org.mockito.ArgumentMatchers.argThat(
                  r -> "SPEED_READER".equals(r.getGameType()) && USER_ID.equals(r.getUserId())));
    }
  }
  ...
  class GetLeaderboard {
    ...
    void mapsAggregationResultsWithUserEnrichment() {
      ...
      AggregationResults<Document> aggResults =
          new AggregationResults<>(List.of(entry), new Document());
      when(mongoTemplate.aggregate(
              any(Aggregation.class), eq("minigame_results"), eq(Document.class)))
          .thenReturn(aggResults);
      when(userRepository.findAllById(List.of(USER_ID)))
          .thenReturn(
              List.of(User.builder().id(USER_ID).name("QA Tester").avatar("avatar.png").build()));
    ...
    void fallsBackToAnonymousName() {
      Document entry =
          new Document("_id", "ghost-user").append("bestScore", 100).append("bestCombo", 0);
      AggregationResults<Document> aggResults =
          new AggregationResults<>(List.of(entry), new Document());
      when(mongoTemplate.aggregate(
              any(Aggregation.class), eq("minigame_results"), eq(Document.class)))
