...
class NotificationSchedulerServiceTest {
  ...
  void notifiesAllAtRiskUsers() {
    UserStats user1 =
        UserStats.builder().userId("u1").currentStreak(5).lastPracticeTime(Instant.now()).build();
    UserStats user2 =
        UserStats.builder().userId("u2").currentStreak(12).lastPracticeTime(Instant.now()).build();
    ...
    verify(notificationService, times(2))
        .notify(
            anyString(),
            eq(NotificationType.STREAK_REMINDER),
            anyString(),
            anyString(),
            anyString(),
            eq(false));
  ...
  void sendsNothingWhenNoneAtRisk() {
    ...
    verify(notificationService, never())
        .notify(anyString(), any(), anyString(), anyString(), anyString(), anyBoolean());
  }
}
