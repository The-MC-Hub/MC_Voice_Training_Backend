...
**Fixed (2026-07-18).** Áp dụng hướng 2 (bảo vệ tầng rộng, không phụ thuộc phải sửa từng endpoint riêng lẻ): thêm `@ExceptionHandler(IllegalStateException.class)` trong `GlobalExceptionHandler.java`, map về HTTP 401 với `ErrorCode.USER_NOT_AUTHENTICATED`. Vì đây là exception handler toàn cục, fix này tự động áp dụng cho MỌI endpoint gọi `SecurityUtils.getCurrentUserId()` khi chưa đăng nhập — không chỉ `/auth/me` mà cả `/community/leaderboard/me` (đã ghi nhận cùng lỗi ở phần cập nhật 2026-07-18 bên trên) và bất kỳ chỗ nào khác trong codebase có cùng pattern.

Cập nhật test hồi quy: `AuthControllerTest$UpdateSettings.delegatesToUpdateSettings` trước đây pin cứng assertion `is5xxServerError()` (tài liệu hoá đúng hành vi lỗi cũ) — đã sửa thành `isUnauthorized()` để khớp hành vi đúng sau fix. Chạy lại `AuthControllerTest`, `CommunityControllerTest`, `ReportControllerTest` — 27/27 PASS.

Không áp dụng thêm hướng 1 (`@PreAuthorize` riêng từng endpoint) vì hướng 2 đã giải quyết triệt để ở tầng framework, tránh rủi ro bỏ sót nếu dev thêm endpoint mới sau này quên gắn annotation.
